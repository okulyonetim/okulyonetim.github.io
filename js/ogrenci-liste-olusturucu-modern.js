...
function seciliSinif(){return document.getElementById('olSinifSecici')?.value||''}
function ogrenciSayisi(sinifAdi){try{return (window.veliler||[]).filter(v=>{const s=(window.siniflar||[]).find(x=>x.id===v.sinifId);return s&&s.ad===sinifAdi}).length}catch(_){return 0}}
function decorate(){
 ...
 const first=[...root.children].find(x=>x.classList&&x.classList.contains('card')&&!x.classList.contains('ol-modern-hero'));if(first)first.classList.add('ol-class-card');
 ...
 area.querySelectorAll('button').forEach(b=>{const tx=(b.textContent||'').trim().toLocaleLowerCase('tr');if(/pdf|excel|yazdır|yazdir/.test(tx))b.classList.add('ol-export-btn');if(/kaydet|güncelle|guncelle/.test(tx))b.classList.add('ol-save-btn')});
...
function updateSummary(){
 const area=document.getElementById('olCalismaAlani'),sinif=seciliSinif();if(!area||!sinif)return;
 let bar=area.querySelector('.ol-modern-summary');const rows=ogrenciSayisi(sinif);const checks=document.querySelectorAll('#tab-ogretmenListe .ol-sutun-check:checked').length;
 if(!bar){bar=document.createElement('div');bar.className='ol-modern-summary';area.prepend(bar)}
 bar.innerHTML=`<div><span class="ico">${svg.users}</span><span><small>Sınıf</small><b>${sinif}</b></span></div><div><span class="ico blue">${svg.list}</span><span><small>Öğrenci</small><b>${rows}</b></span></div><div><span class="ico amber">${svg.columns}</span><span><small>Seçili sütun</small><b>${checks}</b></span></div>`;
}
function bindObserver(){
 const root=document.getElementById('olIcerik');if(!root||root.dataset.olModernBound==='1')return;root.dataset.olModernBound='1';let q=false;
...
function install(){
 if(typeof window.ogretmenListeSekmesiAc==='function'&&!window.ogretmenListeSekmesiAc.__modern){const old=window.ogretmenListeSekmesiAc;const fn=function(){const r=old.apply(this,arguments);requestAnimationFrame(()=>{decorate();bindObserver()});return r};fn.__modern=true;window.ogretmenListeSekmesiAc=fn}
 if(typeof window.olCalismaAlaniOlustur==='function'&&!window.olCalismaAlaniOlustur.__modern){const old=window.olCalismaAlaniOlustur;const fn=async function(){const r=await old.apply(this,arguments);requestAnimationFrame(decorate);return r};fn.__modern=true;window.olCalismaAlaniOlustur=fn}
 requestAnimationFrame(()=>{decorate();bindObserver()});
}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install,{once:true});else install();setTimeout(install,700);
})();