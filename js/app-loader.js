...
function explicitLevel(key,{fallback='edit'}={}){const{user,role}=permissionSession();if(user?.admin===true)return'edit';const u=permissionSourceLevel(user?.yetkiler,key);if(u)return u;const r=permissionSourceLevel(role?.yetkiler,key);if(r)return r;return permissionNormalize(fallback)||'edit'}
function moduleNameForKey(key){const first=String(key||'').split('.')[0];return registry.has(first)?first:null}
function permissionLevel(key,{fallback='edit'}={}){let level=explicitLevel(key,{fallback});const module=moduleNameForKey(key);if(module&&key!==`module.${module}`){const ceiling=explicitLevel('module.'+module,{fallback:'edit'});if(PERMISSION_RANK[level]>PERMISSION_RANK[ceiling])level=ceiling}return level}
...
function permissionRefresh(){applyNavigation();permissionApply(document);const route=AppStore?.get?.('ui.route');if(route)requestAnimationFrame(()=>permissionApplyModule(route));try{window.dispatchEvent(new CustomEvent('koruk:permissions-changed'))}catch(_){}return true}
