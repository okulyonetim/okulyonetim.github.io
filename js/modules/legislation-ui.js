...
function chatView(){return `<section class="ka-legislation-chat" aria-label="Mevzuata soru sor">
  <div class="ka-legislation-chat__head"><span aria-hidden="true">✦</span><div><small>MEVZUAT ASİSTANI</small><h3>Güncel mevzuatı sor</h3><p>Önce cihazındaki arşiv taranır; AI servisi destekliyorsa resmî internet kaynaklarında güncellik doğrulaması da istenir.</p></div></div>
...
function chatWelcome(){if(history.length||loading)return'';return `<div class="ka-legislation-chat-welcome"><span aria-hidden="true">✦</span><strong>Ne öğrenmek istiyorsun?</strong><p>${rows.length?`${rows.length} mevzuat ve ${totalSections()} bölüm önce taranır; ardından destekleniyorsa resmî web kaynaklarında güncellik doğrulanır.`:'Yerel arşiv boş olsa bile destekleniyorsa resmî web kaynaklarında güncel mevzuat aranabilir.'}</p>${rows.length?`<div class="ka-legislation-prompts"><button type="button" data-legislation-prompt="Bu mevzuatların kapsamı nedir?">Kapsamı nedir?</button><button type="button" data-legislation-prompt="Görev ve sorumluluklarla ilgili hükümler nelerdir?">Görev ve sorumluluklar</button><button type="button" data-legislation-prompt="İzinlerle ilgili hükümler nelerdir?">İzin hükümleri</button></div>`:''}</div>`;}
function renderMessages(){const host=document.getElementById('legislationMessages');if(!host)return;host.innerHTML=chatWelcome()+history.map(m=>`<div class="ka-legislation-message ka-legislation-message--${m.role==='user'?'user':'assistant'}"><div class="ka-legislation-message__avatar" aria-hidden="true">${m.role==='user'?'S':'⚖️'}</div><div class="ka-legislation-message__bubble">${esc(m.text).replace(/\n/g,'<br>')}</div></div>`).join('')+(loading?`<div class="ka-legislation-message ka-legislation-message--assistant"><div class="ka-legislation-message__avatar" aria-hidden="true">⚖️</div><div class="ka-legislation-message__bubble ka-legislation-thinking"><span></span><span></span><span></span><em>Arşiv ve güncel kaynaklar taranıyor…</em></div></div>`:'');host.querySelectorAll('[data-legislation-prompt]').forEach(b=>b.onclick=()=>{const input=document.getElementById('legislationQuestion');if(!input)return;input.value=b.dataset.legislationPrompt||'';input.focus();});host.scrollTop=host.scrollHeight;}
async function send(){const input=document.getElementById('legislationQuestion'),q=(input?.value||'').trim();if(!q||loading)return;history.push({role:'user',text:q});const prior=history.slice(0,-1);input.value='';loading=true;renderMessages();try{const result=await engine().ask(q,prior);history.push({role:'model',text:result.text||'(boş yanıt)'});}catch(e){history.push({role:'model',text:'⚠️ Hata: '+(e?.message||e)});}finally{loading=false;renderMessages()}}
