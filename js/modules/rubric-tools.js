...
async function open(){if(user().admin!==true)return toast('Öğrenci Devamsızlığı yalnız yöneticiler içindir.');cleanup();await global.ShellUI?.routeModule?.('people',{bottom:'menu',title:'Öğrenci Devamsızlığı'});global.PeopleModule?.unmount?.();mounted=true;date=today();subscribe();await render();return true;}
function install(){const groups=global.ShellUI?.MENU_GROUPS;if(!Array.isArray(groups))return false;const group=groups.find(x=>x.key==='people');if(!group)return false;group.items=Array.isArray(group.items)?group.items:[];const i=group.items.findIndex(x=>x?.[3]===PAGE);if(user().admin===true&&i<0)group.items.push(['Öğrenci Devamsızlığı','🚫','people',PAGE]);else if(user().admin!==true&&i>=0)group.items.splice(i,1);return true;}
document.addEventListener('click',e=>{const b=e.target?.closest?.(`[data-ka-menu-page="${PAGE}"]`);if(!b)return;e.preventDefault();e.stopImmediatePropagation();open().catch(err=>{console.error('[StudentAbsence/open]',err);toast('Öğrenci Devamsızlığı açılamadı.');});},true);
