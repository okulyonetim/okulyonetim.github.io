...
function shell(){return `<section class="ka-stack" data-tools-module><div class="ka-row ka-row--between"><div><h2>Araçlar</h2><p class="ka-muted">Okul yönetimi yardımcı araçları cihaz verisinden çalışır.</p></div><span id="toolsCount" class="ka-badge"></span></div><div class="ka-tabs"><button class="ka-tab" data-tools-tab="checklists">Kontrol Listeleri</button><button class="ka-tab" data-tools-tab="map">Harita</button><button class="ka-tab" data-tools-tab="forms">Çizelgeler</button><button class="ka-tab" data-tools-tab="attendance">Devamsızlık</button></div><div id="toolsContent" class="ka-stack"></div></section>`}
...
const FORM_COLS={
  sosyalKulupler:{label:'Sosyal Kulüpler',fields:[{k:'ad',l:'Kulüp Adı',r:true},{k:'ogretmenIdler',l:'Danışman Öğretmen(ler)',t:'teachers'},{k:'ogrenciSayisi',l:'Öğrenci Sayısı',t:'number'}],title:r=>r.ad||'Kulüp'},
  sok:{label:'ŞÖK',fields:[{k:'ad',l:'Toplantı Konusu',r:true},{k:'ogretmenId',l:'Öğretmen',t:'teacher'},{k:'tarih1',l:'1. Dönem Tarihi',t:'date'},{k:'tarih2',l:'2. Dönem Tarihi',t:'date'},{k:'tarih3',l:'Yıl Sonu Tarihi',t:'date'},{k:'notlar',l:'Notlar',t:'textarea'}],title:r=>r.ad||'ŞÖK'},
  zumre:{label:'Zümre',fields:[{k:'brans',l:'Branş',r:true},{k:'sinif',l:'Sınıf'},{k:'ogretmenId',l:'Sorumlu Öğretmen',t:'teacher'},{k:'tarih1',l:'1. Dönem Tarihi',t:'date'},{k:'tarih2',l:'2. Dönem Tarihi',t:'date'},{k:'tarih3',l:'Yıl Sonu Tarihi',t:'date'}],title:r=>`${r.brans||'Zümre'}${r.sinif?' – '+r.sinif:''}`},
  bepPlani:{label:'BEP Planları',fields:[{k:'ogrenciAdi',l:'Öğrenci Adı',r:true},{k:'sinif',l:'Sınıfı'},{k:'ogretmenId',l:'Sorumlu Öğretmen',t:'teacher'},{k:'yillikDersPlaniTarihi',l:'Yıllık Ders Planı Tarihi',t:'date'},{k:'bepPlaniTarihi',l:'BEP Planı Tarihi',t:'date'}],title:r=>r.ogrenciAdi||'BEP'},
  rehberlik:{label:'Rehberlik',fields:[{k:'ad',l:'Konu / Başlık',r:true},{k:'ogretmenId',l:'Rehber Öğretmen',t:'teacher'},{k:'yillikPlanTarihi',l:'Yıllık Plan Tarihi',t:'date'},{k:'notlar',l:'Notlar',t:'textarea'}],title:r=>r.ad||'Rehberlik'},
  maarifRapor:{label:'Maarif Model Raporları',fields:[{k:'ders',l:'Ders',r:true},{k:'sinif',l:'Sınıf'},{k:'ogretmenId',l:'Öğretmen',t:'teacher'}],title:r=>`${r.ders||'Maarif'}${r.sinif?' – '+r.sinif:''}`},
  belirliGunler:{label:'Belirli Günler ve Haftalar',fields:[{k:'ad',l:'Etkinlik Adı',r:true},{k:'tarihBaslangic',l:'Başlangıç Tarihi',t:'date'},{k:'tarihBitis',l:'Bitiş Tarihi',t:'date'},{k:'ogretmenIdler',l:'Görevli Öğretmen(ler)',t:'teachers'},{k:'notlar',l:'Notlar',t:'textarea'}],title:r=>r.ad||'Etkinlik'},
  digerEvrak:{label:'Diğer Evrak',fields:[{k:'baslik',l:'Başlık',r:true},{k:'ogretmenId',l:'Sorumlu',t:'teacher'},{k:'tarih',l:'Tarih',t:'date'},{k:'aciklama',l:'Açıklama',t:'textarea'}],title:r=>r.ad||r.baslik||'Evrak'}
};
let formOpenType=null,formOpenId=null;
function formOgretmenSelect(fieldKey,val,multi){const ogretmenler=arr('ogretmenler').sort((a,b)=>`${a.ad} ${a.soyad}`.localeCompare(`${b.ad} ${b.soyad}`,'tr'));if(multi){const vals=Array.isArray(val)?val:[];return`<select name="${esc(fieldKey)}" multiple size="4">${ogretmenler.map(o=>`<option value="${esc(o.id)}" ${vals.includes(o.id)?'selected':''}>` +`${esc(`${o.ad||''} ${o.soyad||''}`.trim())} (${esc(o.brans||'')})</option>`).join('')}</select><small class="ka-muted">Ctrl/Cmd ile çoklu seçim</small>`}return`<select name="${esc(fieldKey)}"><option value="">— Seçiniz —</option>${ogretmenler.map(o=>`<option value="${esc(o.id)}" ${val===o.id?'selected':''}>${esc(`${o.ad||''} ${o.soyad||''}`.trim())} (${esc(o.brans||'')})</option>`).join('')}</select>`}
function formField(f,item){const v=item?.[f.k]??'';const inp=f.t==='teacher'?formOgretmenSelect(f.k,v,false):f.t==='teachers'?formOgretmenSelect(f.k,v,true):f.t==='textarea'?`<textarea name="${esc(f.k)}">${esc(String(v))}</textarea>`:f.t==='number'?`<input name="${esc(f.k)}" type="number" min="0" value="${esc(String(v))}">`:f.t==='date'?`<input name="${esc(f.k)}" type="date" value="${esc(String(v))}">`:` <input name="${esc(f.k)}" value="${esc(String(v))}" ${f.r?'required':''}>`;return`<label class="ka-field"><span class="ka-field__label">${esc(f.l)}${f.r?' *':''}</span>${inp}</label>`}
function formModal(type,item={}){const cfg=FORM_COLS[type];if(!cfg)return'';return`<div class="ka-modal-backdrop" data-form-modal style="z-index:1300"><form class="ka-modal" id="formCizelgeForm"><div class="ka-modal__header"><h2>${esc(item.id?`${cfg.label} Düzenle`:`Yeni ${cfg.label}`)}</h2></div><div class="ka-modal__body ka-stack"><input type="hidden" name="id" value="${esc(item.id||'')}"><input type="hidden" name="formType" value="${esc(type)}">${cfg.fields.map(f=>formField(f,item)).join('')}</div><div class="ka-modal__footer"><button class="ka-btn ka-btn--secondary" type="button" data-form-modal-kapat>Vazgeç</button><button class="ka-btn" type="submit">Kaydet</button></div></form></div>`}
function formOgretmenAdi(id){const o=arr('ogretmenler').find(x=>x.id===id);return o?`${o.ad||''} ${o.soyad||''}`.trim():'—'}
function formRecordHtml(type,r,cfg){const acik=formOpenId===r.id&&formOpenType===type;return`<article class="ka-card"><div class="ka-card__body"><div class="ka-row ka-row--between"><div class="ka-grow"><strong>${esc(cfg.title(r))}</strong><div class="ka-muted">${[r.ogretmenId?formOgretmenAdi(r.ogretmenId):'',r.tarih||r.tarih1||r.tarihBaslangic||''].filter(Boolean).join(' · ')}</div></div><div class="ka-row" style="gap:4px"><button class="ka-btn ka-btn--sm ka-btn--secondary" type="button" data-form-expand="${esc(r.id)}" data-form-type="${esc(type)}">${acik?'▲':'▼'}</button><button class="ka-btn ka-btn--sm ka-btn--secondary" type="button" data-form-edit="${esc(r.id)}" data-form-type="${esc(type)}">Düzenle</button><button class="ka-btn ka-btn--sm ka-btn--danger" type="button" data-form-del="${esc(r.id)}" data-form-type="${esc(type)}">Sil</button></div></div>${acik?`<div class="ka-stack" style="margin-top:10px">${cfg.fields.map(f=>{const v=r[f.k];if(!v&&v!==0)return'';const lbl=`<span class="ka-muted" style="font-size:11px">${esc(f.l)}:</span>`;const val=f.t==='teachers'&&Array.isArray(v)?v.map(formOgretmenAdi).join(', '):f.t==='teacher'?formOgretmenAdi(v):String(v);return val?`<div>${lbl} ${esc(val)}</div>`:''}).filter(Boolean).join('')}</div>`:''}</div></article>`}
function renderForms(){const types=FORM_TYPES,content=document.getElementById('toolsContent'),count=document.getElementById('toolsCount');const total=types.reduce((n,t)=>n+arr(t).length,0);if(count)count.textContent=`${total} kayıt`;if(!content)return;content.innerHTML=types.map(type=>{const cfg=FORM_COLS[type]||{label:FORM_LABELS[type]||type,title:r=>r.ad||r.baslik||type,fields:[]};const rows=arr(type);return`<section class="ka-stack"><div class="ka-row ka-row--between" style="margin-bottom:4px"><h3 style="font-size:14px">${esc(cfg.label||FORM_LABELS[type]||type)}<span class="ka-badge" style="margin-left:6px">${rows.length}</span></h3><button class="ka-btn ka-btn--sm ka-btn--secondary" type="button" data-form-add="${esc(type)}">+ Ekle</button></div>${rows.length?rows.map(r=>formRecordHtml(type,r,cfg)).join(''):'<div class="ka-empty">Kayıt yok.</div>'}</section>`}).join('');bindForms(content)}
function bindForms(root){if(!root)return;root.querySelectorAll('[data-form-add]').forEach(b=>b.addEventListener('click',()=>{document.querySelector('[data-form-modal]')?.remove();document.body.insertAdjacentHTML('beforeend',formModal(b.dataset.formAdd,{}));_bindFormModal()}));root.querySelectorAll('[data-form-edit]').forEach(b=>b.addEventListener('click',()=>{const t=b.dataset.formType,id=b.dataset.formEdit,item=arr(t).find(x=>x.id===id)||{};document.querySelector('[data-form-modal]')?.remove();document.body.insertAdjacentHTML('beforeend',formModal(t,item));_bindFormModal()}));root.querySelectorAll('[data-form-del]').forEach(b=>b.addEventListener('click',async()=>{if(!confirm('Bu kayıt silinsin mi?'))return;const t=b.dataset.formType,id=b.dataset.formDel,col=COL?.[t];if(!col||!global.DeviceData)return;try{await global.DeviceData.remove(t,col,id);render()}catch(e){if(typeof toast==='function')toast(e?.message||'Silinemedi')}}));root.querySelectorAll('[data-form-expand]').forEach(b=>b.addEventListener('click',()=>{const id=b.dataset.formExpand,t=b.dataset.formType;formOpenId=formOpenId===id&&formOpenType===t?null:id;formOpenType=t;renderForms()}))}
function _bindFormModal(){const modal=document.querySelector('[data-form-modal]');if(!modal)return;modal.querySelector('[data-form-modal-kapat]')?.addEventListener('click',()=>modal.remove());modal.addEventListener('click',e=>{if(e.target===modal)modal.remove()});modal.querySelector('#formCizelgeForm')?.addEventListener('submit',async e=>{e.preventDefault();const fd=new FormData(e.currentTarget),id=fd.get('id')||'',type=fd.get('formType')||'';const cfg=FORM_COLS[type];if(!cfg){modal.remove();return}const col=COL?.[type];if(!col||!global.DeviceData){if(typeof toast==='function')toast('Koleksiyon bulunamadı');return}const v={};cfg.fields.forEach(f=>{if(f.t==='teachers'){const vals=fd.getAll(f.k);v[f.k]=vals.filter(Boolean)}else if(f.t==='number'){v[f.k]=Number(fd.get(f.k))||0}else{v[f.k]=String(fd.get(f.k)||'').trim()}});try{if(id)await global.DeviceData.update(type,col,id,v);else await global.DeviceData.add(type,col,{...v,olusturmaTarihi:new Date().toISOString()});if(typeof toast==='function')toast('Kaydedildi.');modal.remove();formOpenId=null;render()}catch(err){if(typeof toast==='function')toast(err?.message||'Kaydedilemedi')}})}
...
function render(){if(!mounted)return;document.querySelectorAll('[data-tools-tab]').forEach(b=>b.classList.toggle('active',b.dataset.toolsTab===active));if(active==='map')renderMap();else if(active==='forms')renderForms();else if(active==='attendance')renderAttendance();else renderChecklists()}
...
function bind(){document.querySelectorAll('[data-tools-tab]').forEach(b=>b.onclick=async()=>{active=b.dataset.toolsTab;if(active==='map')await global.ToolsData?.prepareMap?.();if(active==='forms')await global.ToolsData?.prepareForms?.();if(active==='attendance')await global.ToolsData?.prepareAttendance?.();render()})}
function subscribe(){unsubs.forEach(f=>{try{f()}catch(_){}});unsubs=[];const paths=['data.kontrolListeleri','data.kontrolListeTamamlama','data.haritaFavoriler','data.servisler','data.devamsizlikCizelgesi',...(global.ToolsData?.FORM_TYPES||[]).map(t=>'data.'+t)];paths.forEach(p=>{const u=global.AppStore?.subscribe?.(p,()=>requestAnimationFrame(render));if(u)unsubs.push(u)})}
...
async function renderAttendance(){const r=root();if(!r)return;const classes=await accessibleAttendanceClasses();ensureClass(classes);const students=studentsOf(selectedClass),doc=await attendanceRecord(),states=doc?.kayitlar||{};r.innerHTML=`<section class="ka-stack" data-student-page="attendance">${pageHead('Öğrenci Yoklama','Yoklama cihazdaki veriden anında açılır; değişiklikler arka planda senkronize edilir.',`<span class="ka-badge">${students.length} öğrenci</span>`)}<div class="ka-card"><div class="ka-card__body ka-grid"><label class="ka-field"><span class="ka-field__label">Sınıf</span><select id="studentAttendanceClass">${classOptions(classes)}</select></label><label class="ka-field"><span class="ka-field__label">Tarih</span><input id="studentAttendanceDate" type="date" value="${esc(selectedDate)}"></label></div></div>${selectedClass?`<div class="ka-row ka-row--between"><strong>${esc(className(classes.find(x=>x.id===selectedClass)))}</strong><button class="ka-btn ka-btn--secondary ka-btn--sm" type="button" data-att-all-present ${students.length?'':'disabled'}>Tümünü Var İşaretle</button></div>`:''}<div class="ka-stack">${students.length?students.map((s,i)=>`<article class="ka-card ka-list-card"><div class="ka-card__body ka-row"><span class="ka-badge">${esc(s.ogrenciNo||s.no||i+1)}</span><div class="ka-grow"><strong>${esc(fullStudent(s))}</strong><div class="ka-muted">${esc(s.cinsiyet||'')}</div></div><label class="ka-field"><select data-att-student="${esc(s.id)}">${attendanceStatusOptions(states[s.id]||'var')}</select></label></div></article>`).join(''):empty(selectedClass?'Bu sınıfta öğrenci bulunamadı.':'Sınıf seçiniz.')}</div></section>`;$('#studentAttendanceClass')?.addEventListener('change',e=>{selectedClass=e.target.value;renderAttendance()});$('#studentAttendanceDate')?.addEventListener('change',e=>{selectedDate=e.target.value||today();renderAttendance()});$$('[data-att-student]',r).forEach(sel=>sel.addEventListener('change',async()=>{sel.disabled=true;try{await global.YoklamaService.ogrenciDurumKaydet(selectedClass,selectedDate,sel.dataset.attStudent,sel.value);toast('Yoklama güncellendi.')}catch(e){toast(e?.message==='yetkisiz'?'Bu işlem için yetkiniz yok.':'Yoklama kaydedilemedi.');await renderAttendance()}finally{sel.disabled=false}}));$('[data-att-all-present]',r)?.addEventListener('click',async e=>{const b=e.currentTarget,before=b.textContent;b.disabled=true;b.textContent='Kaydediliyor…';try{const kayitlar=Object.fromEntries(students.map(s=>[s.id,'var']));await global.YoklamaService.yoklamaKaydet(selectedClass,selectedDate,kayitlar);toast('Tüm öğrenciler var işaretlendi.')}catch(err){toast(err?.message==='yetkisiz'?'Bu işlem için yetkiniz yok.':'Yoklama kaydedilemedi.')}finally{b.disabled=false;b.textContent=before}})}
