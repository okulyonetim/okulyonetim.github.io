...
function segmented(marker,values,current){return `<div class="ka-meeting-segmented">${values.map(([v,label])=>`<button type="button" class="ka-meeting-segment ${current===v?'is-active':''}" ${marker}="${esc(v)}" aria-pressed="${current===v?'true':'false'}">${esc(label)}</button>`).join('')}</div>`}
