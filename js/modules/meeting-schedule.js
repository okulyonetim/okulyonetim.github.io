...
function rowDetailField(row,index){
  if(common.tur==='zumre'&&common.kademe==='ortaokul'){
    return `<select aria-label="${index+1}. satır ders" data-meeting-row-lesson="${esc(row.uid)}"><option value="">Ders seçiniz</option>${lessons().map(d=>`<option value="${esc(d.id)}" ${row.dersId===d.id?'selected':''}>${esc(d.ad)}</option>`).join('')}</select>`;
  }
  if(common.tur==='zumre'&&common.kademe==='ilkokul'){
    return `<select aria-label="${index+1}. satır sınıf düzeyi" data-meeting-row-level="${esc(row.uid)}"><option value="">Düzey seçiniz</option>${[1,2,3,4].map(n=>`<option value="${n}" ${Number(row.seviye)===n?'selected':''}>${n}. Sınıf</option>`).join('')}</select>`;
  }
  return `<input aria-label="${index+1}. satır konu veya gündem" data-meeting-row-topic="${esc(row.uid)}" value="${esc(row.satirKonusu)}" placeholder="${common.tur==='sok'?'Örn. Akademik başarı':'Konu / açıklama'}">`;
}
function classSelectionTags(row){
  if(row.tumSiniflar)return'<span class="ka-badge">Tüm Sınıflar</span>';
  const map=new Map(classes().map(s=>[s.id,className(s)])),names=row.sinifIdler.map(id=>map.get(id)).filter(Boolean);
  return names.length?names.map(n=>`<span class="ka-badge">${esc(n)} ×</span>`).join(''):'<span class="ka-meeting-class-placeholder">Sınıf seçiniz</span>';
}
function classDropdown(row){
  const list=eligibleClasses(row),selected=row.tumSiniflar?[]:row.sinifIdler.filter(Boolean),open=openClassRowId===row.uid;
...
function rowHtml(row,index){
  return `<tr data-meeting-row="${esc(row.uid)}"><td class="ka-meeting-col-detail">${rowDetailField(row,index)}</td><td class="ka-meeting-col-classes">${classDropdown(row)}</td><td class="ka-meeting-col-date"><input aria-label="${index+1}. satır tarih" type="date" data-meeting-row-date="${esc(row.uid)}" value="${esc(row.tarih)}"></td><td class="ka-meeting-col-time"><input aria-label="${index+1}. satır saat" type="time" data-meeting-row-time="${esc(row.uid)}" value="${esc(row.saat)}"></td><td class="ka-meeting-col-actions"><button type="button" class="ka-btn ka-btn--ghost ka-btn--sm ka-meeting-delete" data-meeting-remove-row="${esc(row.uid)}" ${rows.length===1?'disabled':''} aria-label="${index+1}. satırı sil">🗑</button></td></tr>${classPanelRow(row)}`;
}
function formHtml(){
  const editMode=!!editingGroupId||!!editingLegacyKey;
  return `<article class="ka-card ka-meeting-form-card"><div class="ka-card__body ka-stack"><div class="ka-row ka-row--between"><div><h3>${editMode?'Toplantı Grubunu Düzenle':'Toplantı Bilgileri (Ortak)'}</h3><p class="ka-muted">Ortak bilgileri bir kez girin; toplantıları aşağıdaki tabloya satır satır ekleyin.</p></div>${editMode?'<span class="ka-badge">Düzenleme</span>':''}</div><div class="ka-grid ka-meeting-date-grid"><label class="ka-field"><span class="ka-field__label">Toplantı Türü</span><select data-meeting-type-select>${[['sok','ŞÖK Toplantısı'],['zumre','Zümre Toplantısı'],['diger','Diğer']].map(([v,label])=>`<option value="${v}" ${common.tur===v?'selected':''}>${label}</option>`).join('')}</select></label><label class="ka-field"><span class="ka-field__label">Kademe</span><select data-meeting-kademe-select><option value="ilkokul" ${common.kademe==='ilkokul'?'selected':''}>İlkokul</option><option value="ortaokul" ${common.kademe==='ortaokul'?'selected':''}>Ortaokul</option></select></label></div><label class="ka-field"><span class="ka-field__label">Toplantı Başlığı <small class="ka-muted">(ortak)</small></span><input data-meeting-common-title value="${esc(common.baslik)}" placeholder="Örn. 2. Dönem Zümre Toplantıları"></label><div class="ka-row ka-row--between"><div><h3>Toplantı Detayları (Satırlar)</h3><p class="ka-muted">Her satırda farklı ders/konu, sınıf, tarih ve saat seçilebilir.</p></div><button type="button" class="ka-btn ka-btn--secondary" data-meeting-add-row ${saving?'disabled':''}>＋ Satır Ekle</button></div><div class="ka-table-wrap"><table class="ka-table"><thead><tr><th class="ka-meeting-col-detail">Ders / Konu</th><th class="ka-meeting-col-classes">Sınıflar <small>(isteğe bağlı)</small></th><th class="ka-meeting-col-date">Tarih</th><th class="ka-meeting-col-time">Saat</th><th class="ka-meeting-col-actions">İşlemler</th></tr></thead><tbody>${rows.map(rowHtml).join('')}</tbody></table></div><small class="ka-muted">ⓘ Sınıf seçimi isteğe bağlıdır. Birden fazla sınıf seçebilir veya boş bırakabilirsiniz.</small>${formMessage?`<div class="ka-card" data-meeting-form-message role="alert"><div class="ka-card__body"><strong>⚠️ ${esc(formMessage)}</strong></div></div>`:''}<div class="ka-row"><button type="button" class="ka-btn ka-btn--secondary ka-grow" data-meeting-reset ${saving?'disabled':''}>${editMode?'Vazgeç':'Temizle'}</button><button type="button" class="ka-btn ka-grow" data-meeting-save-all ${saving?'disabled':''}>💾 ${saving?'Kaydediliyor…':`Tümünü Kaydet (${rows.length})`}</button></div></div></article>`;
}
