...
function classSelectionTags(row){
  if(row.tumSiniflar)return'<span class="ka-meeting-class-summary"><strong>Tüm Sınıflar</strong></span>';
  const map=new Map(classes().map(s=>[s.id,className(s)])),names=row.sinifIdler.map(id=>map.get(id)).filter(Boolean);
  if(!names.length)return'<span class="ka-meeting-class-placeholder">Sınıf seçiniz</span>';
  const summary=names.slice(0,2).join(', ')+(names.length>2?` +${names.length-2}`:'');
  return `<span class="ka-meeting-class-summary"><strong>${esc(summary)}</strong></span>`;
}
...
function classPanelHtml(row){
  if(openClassRowId!==row.uid)return'';
  const list=eligibleClasses(row),selected=row.tumSiniflar?[]:row.sinifIdler.filter(Boolean);
  return `<div class="ka-meeting-class-panel" data-meeting-class-panel="${esc(row.uid)}"><div class="ka-row ka-row--between"><strong>Sınıflar</strong><button type="button" class="ka-btn ka-btn--ghost ka-btn--sm" data-meeting-close-classes="${esc(row.uid)}">Bitti</button></div><div class="ka-meeting-class-grid"><button type="button" class="ka-meeting-chip ka-meeting-chip--all ${row.tumSiniflar?'is-active':''}" data-meeting-row-all-classes="${esc(row.uid)}" aria-pressed="${row.tumSiniflar?'true':'false'}">Tüm Sınıflar</button>${list.map(s=>{const on=!row.tumSiniflar&&selected.includes(s.id);return`<button type="button" class="ka-meeting-chip ${on?'is-active':''}" data-meeting-row-class="${esc(row.uid)}" data-class-id="${esc(s.id)}" aria-pressed="${on?'true':'false'}">${esc(className(s))}</button>`}).join('')}</div>${selected.length||row.tumSiniflar?`<button type="button" class="ka-btn ka-btn--ghost ka-btn--sm" data-meeting-clear-classes="${esc(row.uid)}">Seçimi Temizle</button>`:''}${list.length?'':'<small class="ka-muted">Bu satıra uygun sınıf kaydı bulunamadı.</small>'}</div>`;
}
function draftIcon(name){
  const icons={
    tag:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20 13 13 20 4 11V4h7l9 9Z"/><circle cx="8.5" cy="8.5" r="1.2"/></svg>',
    users:'<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="9" cy="8" r="3"/><path d="M3.5 19c.5-3.2 2.4-5 5.5-5s5 1.8 5.5 5M16 6.5a2.5 2.5 0 1 1 0 5M16.5 14c2.3.2 3.6 1.7 4 4"/></svg>',
    calendar:'<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="4" y="5" width="16" height="15" rx="2"/><path d="M8 3v4M16 3v4M4 10h16"/></svg>',
    clock:'<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="8"/><path d="M12 7v5l3.5 2"/></svg>',
    trash:'<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M9 7V4h6v3M7 7l1 13h8l1-13M10 11v5M14 11v5"/></svg>'
  };
  return icons[name]||'';
}
function draftField(icon,label,control,extra=''){
  return `<div class="ka-meeting-draft-field ${extra}"><span class="ka-meeting-draft-field__icon" aria-hidden="true">${draftIcon(icon)}</span><div class="ka-meeting-draft-field__body"><span class="ka-meeting-draft-field__label">${esc(label)}</span>${control}</div></div>`;
}
function rowHtml(row,index){
  const detail=draftField('tag','Ders / Konu',rowDetailField(row,index),'ka-meeting-draft-field--detail');
  const sinif=draftField('users','Sınıflar',classDropdown(row),'ka-meeting-draft-field--classes');
  const tarih=draftField('calendar','Tarih',`<input class="ka-meeting-date-control" aria-label="${index+1}. satır tarih" type="date" data-meeting-row-date="${esc(row.uid)}" value="${esc(row.tarih)}">`,'ka-meeting-draft-field--date');
  const saat=draftField('clock','Saat',`<input class="ka-meeting-time-control" aria-label="${index+1}. satır saat" type="time" data-meeting-row-time="${esc(row.uid)}" value="${esc(row.saat)}">`,'ka-meeting-draft-field--time');
  return `<article class="ka-meeting-draft-card" data-meeting-row="${esc(row.uid)}"><div class="ka-meeting-draft-card__head"><span class="ka-meeting-row-badge">Satır ${index+1}</span><button type="button" class="ka-meeting-row-delete" data-meeting-remove-row="${esc(row.uid)}" ${rows.length===1?'disabled':''} aria-label="${index+1}. satırı sil">${draftIcon('trash')}<span>Sil</span></button></div><div class="ka-meeting-draft-grid">${detail}${sinif}${tarih}${saat}</div>${classPanelHtml(row)}</article>`;
}
function formHtml(){
  const editMode=!!editingGroupId||!!editingLegacyKey;
  return `<article class="ka-card ka-meeting-form-card"><div class="ka-card__body ka-stack"><div class="ka-row ka-row--between ka-meeting-form-head"><div><h3>${editMode?'Toplantı Grubunu Düzenle':'Toplantı Bilgileri (Ortak)'}</h3><p class="ka-muted">Ortak bilgileri bir kez girin; toplantıları aşağıdaki kartlara satır satır ekleyin.</p></div>${editMode?'<span class="ka-badge">Düzenleme</span>':''}</div><div class="ka-grid ka-meeting-common-grid"><label class="ka-field"><span class="ka-field__label">Toplantı Türü</span><select data-meeting-type-select>${[['sok','ŞÖK Toplantısı'],['zumre','Zümre Toplantısı'],['diger','Diğer']].map(([v,label])=>`<option value="${v}" ${common.tur===v?'selected':''}>${label}</option>`).join('')}</select></label><label class="ka-field"><span class="ka-field__label">Kademe</span><select data-meeting-kademe-select><option value="ilkokul" ${common.kademe==='ilkokul'?'selected':''}>İlkokul</option><option value="ortaokul" ${common.kademe==='ortaokul'?'selected':''}>Ortaokul</option></select></label></div><label class="ka-field"><span class="ka-field__label">Toplantı Başlığı <small class="ka-muted">(ortak)</small></span><input data-meeting-common-title value="${esc(common.baslik)}" placeholder="Örn. 2. Dönem Zümre Toplantıları"></label><div class="ka-row ka-row--between ka-meeting-detail-head"><div><h3>Toplantı Detayları</h3><p class="ka-muted">Her kartta farklı ders/konu, sınıf, tarih ve saat seçilebilir.</p></div><button type="button" class="ka-btn ka-btn--secondary" data-meeting-add-row ${saving?'disabled':''}>＋ Satır Ekle</button></div><div class="ka-meeting-draft-list">${rows.map(rowHtml).join('')}</div><div class="ka-meeting-info">ⓘ Sınıf seçimi isteğe bağlıdır. Birden fazla sınıf seçebilir veya boş bırakabilirsiniz.</div>${formMessage?`<div class="ka-card" data-meeting-form-message role="alert"><div class="ka-card__body"><strong>⚠️ ${esc(formMessage)}</strong></div></div>`:''}<div class="ka-meeting-form-actions"><button type="button" class="ka-btn ka-btn--secondary" data-meeting-reset ${saving?'disabled':''}>${editMode?'Vazgeç':'Temizle'}</button><button type="button" class="ka-btn" data-meeting-save-all ${saving?'disabled':''}>💾 ${saving?'Kaydediliyor…':`Tümünü Kaydet (${rows.length})`}</button></div></div></article>`;
}
