...
function setKademe(k){if(!['ilkokul','ortaokul'].includes(k)||k===draft.kademe)return;clearFormMessage();draft.kademe=k;draft.zumreSeviyeleri=[];draft.bransId='';draft.bransAdi='';draft.tumSiniflar=false;draft.sinifIdler=[];render()}
function setType(t){if(!['sok','zumre','diger'].includes(t)||t===draft.tur)return;clearFormMessage();draft.tur=t;if(t!=='zumre'){draft.zumreSeviyeleri=[];draft.bransId='';draft.bransAdi=''}draft.tumSiniflar=false;draft.sinifIdler=[];render()}
function toggleLevel(n){clearFormMessage();n=Number(n);const s=new Set(draft.zumreSeviyeleri);s.has(n)?s.delete(n):s.add(n);draft.zumreSeviyeleri=[...s].sort((a,b)=>a-b);draft.tumSiniflar=false;pruneClasses();render()}
function toggleClass(id){clearFormMessage();draft.tumSiniflar=false;const s=new Set(draft.sinifIdler);s.has(id)?s.delete(id):s.add(id);draft.sinifIdler=[...s];render()}
