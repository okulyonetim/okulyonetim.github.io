...
function shell(){return `<section class="ka-stack" data-transport-module><div class="ka-row ka-row--between"><div><h2>Taşıma</h2><p class="ka-muted" data-transport-subtitle>Servisler, servis oturma planları ve taşıma işlemleri.</p></div><span id="transportCount" class="ka-badge"></span></div><label class="ka-field"><span class="ka-field__label">Ara</span><input id="transportSearch" type="search" placeholder="Servis, plaka, güzergâh veya sınıf ara…"></label><div id="transportContent" class="ka-stack"></div></section>`}
...
function render(){if(!mounted)return;const r=active==='foodDaily'?foodMenuPage('foodDaily'):active==='foodWeekly'?foodMenuPage('foodWeekly'):active==='foodMonthly'?foodMenuPage('foodMonthly'):active==='food'?foodPage():active==='services'&&serviceDetailId?serviceDetail():active==='busSeats'?busSeats():active==='classSeats'?classSeats():services(),out=document.getElementById('transportContent'),c=document.getElementById('transportCount'),search=document.getElementById('transportSearch');const h=document.querySelector('[data-transport-module] > .ka-row h2');const sub=document.querySelector('[data-transport-module] [data-transport-subtitle]');if(h){const titles={services:'Taşıma',busSeats:'Servis Oturma',classSeats:'Sınıf Oturma',foodDaily:'Günlük Menü',foodWeekly:'Haftalık Menü',foodMonthly:'Aylık Menü',food:'Yemek Denetim Formu'};h.textContent=titles[active]||'Taşıma'}if(sub){const subtitles={services:'Servisler ve taşıma işlemleri.',busSeats:'Servis oturma planları.',classSeats:'Sınıf oturma planları.',foodDaily:'Günlük yemek menüsünü görüntüleyin.',foodWeekly:'Haftalık yemek menüsünü görüntüleyin.',foodMonthly:'Aylık yemek menüsünü düzenleyin.',food:'Yemek denetim ve kontrol formu.'};sub.textContent=subtitles[active]||'Taşıma işlemleri.'}if(out)out.innerHTML=r.html;if(c)c.textContent=serviceDetailId?`${r.count} öğrenci`:`${r.count} kayıt`;if(search)search.closest('.ka-field').hidden=!!serviceDetailId||['foodDaily','foodWeekly','foodMonthly','food'].includes(active);if(serviceDetailId){bindServiceDetail(out);window.PermissionService?.apply?.(document.getElementById('v2ModuleRoot')||document);return}if(['foodDaily','foodWeekly','foodMonthly'].includes(active)){
...
function openPage(page,title=''){
 const allowed=['services','busSeats','classSeats','foodDaily','foodWeekly','foodMonthly','food'];
 const titlePage={'Günlük Menü':'foodDaily','Haftalık Menü':'foodWeekly','Aylık Menü':'foodMonthly','Yemek Denetim Formu':'food'};
 const resolved=allowed.includes(page)?page:(titlePage[String(title||'').trim()]||'');
 if(!allowed.includes(resolved))return false;
 active=resolved;query='';serviceDetailId='';
 const h=document.querySelector('[data-transport-module] > .ka-row h2');
 if(h)h.textContent=title||({foodDaily:'Günlük Menü',foodWeekly:'Haftalık Menü',foodMonthly:'Aylık Menü',food:'Yemek'}[resolved]||'Taşıma');
 render();return true}
