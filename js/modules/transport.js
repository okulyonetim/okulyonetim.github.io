...
function shell(){return `<section class="ka-stack" data-transport-module><div class="ka-row ka-row--between"><div><h2>Taşıma & Yemek</h2><p class="ka-muted">Taşıma, oturma planları ve yemek işlemleri tek bölümden yönetilir.</p></div><span id="transportCount" class="ka-badge"></span></div><nav class="ka-row ka-wrap transport-module-nav" aria-label="Taşıma ve yemek"><button class="ka-btn ka-btn--sm" type="button" data-transport-page="services">🚌 Servisler</button><button class="ka-btn ka-btn--sm ka-btn--secondary" type="button" data-transport-page="busSeats">💺 Servis Oturma</button><button class="ka-btn ka-btn--sm ka-btn--secondary" type="button" data-transport-page="foodDaily">🍽️ Günlük Menü</button><button class="ka-btn ka-btn--sm ka-btn--secondary" type="button" data-transport-page="foodWeekly">📆 Haftalık Menü</button><button class="ka-btn ka-btn--sm ka-btn--secondary" type="button" data-transport-page="foodMonthly">🗓️ Aylık Menü</button><button class="ka-btn ka-btn--sm ka-btn--secondary" type="button" data-transport-page="food">📋 Yemek Denetimi</button></nav><label class="ka-field"><span class="ka-field__label">Ara</span><input id="transportSearch" type="search" placeholder="Servis, plaka, güzergâh veya sınıf ara…"></label><div id="transportContent" class="ka-stack"></div></section>`}
...
function busPrintReport(s){
  ...
  const extra='<style>.sbe-print{font-family:Arial,sans-serif;color:#18241f;font-size:9pt}.sbe-print h1,.sbe-print h2{text-align:center;margin:2mm 0}.sbe-print p{margin:2mm 0;font-size:8pt}.sbe-print-table{display:grid;width:min(170mm,100%);margin:5mm auto 0;border:1.5px solid #50605a;border-radius:7mm;overflow:hidden;background:#eef2f0;box-sizing:border-box;break-inside:avoid}.sbe-print-cell{box-sizing:border-box;min-width:0;min-height:0;border-right:1px solid #789088;border-bottom:1px solid #789088;background:#fff;padding:1.5mm 1mm;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;overflow:hidden}.sbe-print-empty{background:#eef2f0}.sbe-print-cell.seat{background:#f8fbfa}.sbe-print-cell.filled{background:#d7f1e8}.sbe-print-cell.driver{background:#fff3d6}.sbe-print-cell.door{background:#e9eef2}.sbe-print-cell.special{background:#eef2f0}.sbe-print-no{align-self:flex-start;font-size:6.5pt;font-weight:400}.sbe-print-icon{font-size:13pt;line-height:1}.sbe-print-cell strong{display:block;width:100%;font-size:7.2pt;line-height:1.08;max-width:100%;overflow-wrap:anywhere;word-break:break-word;white-space:normal}.sbe-print-cell small{font-size:5.8pt;margin-top:1mm}</style>';
...
function foodMenuCalendarRows(key,data){
 ...
 for(let w=0;w<weeks;w++){
  ...
  for(let ci=0;ci<5;ci++){
   ...
   if(!day){rows+='<div class="food-calendar-cell is-empty"></div>';continue}
...
function foodMenuPage(mode){
 ...
 if(mode==='foodWeekly'){
  ...
  const rows=Array.from({length:5},(_,i)=>{
   ...
   const iso=d.toISOString().slice(0,10),k=foodMenuMonthKey(iso),items=foodDayItems(foodMenuData(k)[d.getDate()]||{});
   const menu=items.length?items.map((v,j)=>'<div class="food-week-item"><span>'+(j+1)+'.</span>'+esc(v)+'</div>').join(''):'<span class="ka-muted">Menü girilmemiş</span>';
   return '<tr><th><div class="food-week-day">'+esc(d.toLocaleDateString('tr-TR',{weekday:'long'}))+'<small>'+esc(d.toLocaleDateString('tr-TR',{day:'2-digit',month:'2-digit',year:'numeric'}))+'</small></div></th><td>'+menu+'</td></tr>';
  }).join('');
  return{count:5,html:'<section class="ka-stack" data-food-menu-page data-food-menu-mode="foodWeekly"><style>.food-week-title{margin:0}.food-week-table th{width:145px;vertical-align:top}.food-week-day{font-size:14px}.food-week-day small{display:block;margin-top:4px;font-weight:400;opacity:.7}.food-week-item{padding:5px 0;border-bottom:1px solid var(--ka-border,#e4e8e6)}.food-week-item:last-child{border-bottom:0}.food-week-item span{display:inline-block;min-width:24px;font-weight:700}</style><div class="ka-row ka-row--between ka-wrap"><div><h3 class="food-week-title">📆 Haftalık Menü</h3><p class="ka-muted">'+esc(weekTitle)+'</p><p class="ka-muted">Bu ekran aylık menüde girilen bilgileri gösterir.</p></div><div class="ka-row"><input class="ka-input" type="date" data-fm-week-date value="'+esc(viewDate)+'"><button class="ka-btn" type="button" data-fm-print>🖨 A4</button></div></div><div class="ka-card"><div class="ka-card__body"><div class="ka-table-wrap"><table class="ka-table food-week-table"><thead><tr><th>Gün</th><th>Yemekler</th></tr></thead><tbody>'+rows+'</tbody></table></div></div></div></section>'};
 }
...
function render(){if(!mounted)return;const r=active==='foodDaily'?foodMenuPage('foodDaily'):active==='foodWeekly'?foodMenuPage('foodWeekly'):active==='foodMonthly'?foodMenuPage('foodMonthly'):active==='food'?foodPage():active==='services'&&serviceDetailId?serviceDetail():active==='busSeats'?busSeats():active==='classSeats'?classSeats():services(),out=document.getElementById('transportContent'),c=document.getElementById('transportCount'),search=document.getElementById('transportSearch');const h=document.querySelector('[data-transport-module] > .ka-row h2');if(h){const titles={services:'Taşıma',busSeats:'Servis Oturma',foodDaily:'Günlük Menü',foodWeekly:'Haftalık Menü',foodMonthly:'Aylık Menü',food:'Yemek'};h.textContent=titles[active]||'Taşıma & Yemek'}const transportRoot=document.querySelector('[data-transport-module]');if(transportRoot){const isFood=['foodDaily','foodWeekly','foodMonthly','food'].includes(active);const head=transportRoot.querySelector(':scope > .ka-row');const nav=transportRoot.querySelector('.transport-module-nav');const search=transportRoot.querySelector('#transportSearch')?.closest('.ka-field');if(head)head.classList.toggle('food-shell-head',isFood);if(nav)nav.hidden=isFood;if(search&&isFood)search.closest('.ka-field').hidden=true;const desc=head?.querySelector('.ka-muted');if(desc)desc.hidden=isFood;const badge=head?.querySelector('#transportCount');if(badge)badge.hidden=isFood}if(out)out.innerHTML=r.html;if(c)c.textContent=serviceDetailId?`${r.count} öğrenci`:`${r.count} kayıt`;if(search)search.closest('.ka-field').hidden=!!serviceDetailId||active==='food';if(serviceDetailId){bindServiceDetail(out);window.PermissionService?.apply?.(document.getElementById('v2ModuleRoot')||document);return}if(['foodDaily','foodWeekly','foodMonthly'].includes(active)){
 ...
 if(fm){
  ...
  fm.querySelectorAll('[data-fm-item]').forEach(inp=>inp.addEventListener('input',()=>{
    ...
    foodDayEnsure(d);
    const rows=[...fm.querySelectorAll('[data-fm-item="'+day+'"]')];
    d.items=rows.map(x=>x.value.trim()).filter(Boolean);
    foodMenuSave(foodMenuLoad());
  }));
  fm.querySelectorAll('[data-fm-add]').forEach(btn=>btn.addEventListener('click',()=>{
    ...
    requestAnimationFrame(()=>fm.querySelectorAll('[data-fm-item="'+day+'"]')[d.items.length-1]?.focus());
  }));
  fm.querySelectorAll('[data-fm-remove]').forEach(btn=>btn.addEventListener('click',()=>{
