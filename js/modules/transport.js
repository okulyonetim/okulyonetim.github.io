...
async function prepareLocal(){if(!window.SyncEngine||!window.COL)return;const defs={servisOturma:COL.servisOturma,sinifOturma:COL.sinifOturma,resmiTatiller:COL.resmiTatiller},types=[];Object.entries(defs).forEach(([t,c])=>{if(c){SyncEngine.register(t,c);types.push(t)}});await SyncEngine.localHydrate(types);if(types.length)SyncEngine.schedule(100)}
function shell(){return `<section class="ka-stack ka-transport-page" data-transport-module>\n<div class="ka-transport-page-hero"><div class="ka-transport-page-hero__icon" aria-hidden="true">🚌</div><div class="ka-transport-page-hero__copy"><span class="ka-transport-page-hero__eyebrow">TAŞIMA YÖNETİMİ</span><h2>Servisler ve taşıma</h2><p>Servisleri, öğrencileri ve oturma planlarını tek merkezden yönetin.</p></div><span id="transportCount" class="ka-transport-page-hero__count">0 kayıt</span></div>\n<div class="ka-transport-page-stats" aria-label="Taşıma özeti"><div><span>TOPLAM SERVİS</span><strong id="transportTotalCount">0</strong></div><div><span>AKTİF SERVİS</span><strong id="transportActiveCount">0</strong></div><div><span>TAŞINAN ÖĞRENCİ</span><strong id="transportStudentCount">0</strong></div></div>\n<nav class="transport-module-nav ka-transport-page-tabs" aria-label="Taşıma"><button class="ka-btn" type="button" data-transport-page="services">🚌 Servisler</button><button class="ka-btn ka-btn--secondary" type="button" data-transport-page="busSeats">💺 Servis Oturma</button></nav>\n<label class="ka-field ka-transport-search"><span class="ka-field__label">Servis ara</span><input id="transportSearch" type="search" placeholder="Servis, plaka, güzergâh veya sınıf ara…"></label><div id="transportContent" class="ka-stack"></div></section>`}
...
function sbeHistoryInit(){const snap=sbeSnapshot();editor.history=[snap];editor.historyIndex=0;editor.undo=[];editor.redo=[]}
function sbePush(snapshot=null){if(!editor?.editable)return;const snap=snapshot||sbeSnapshot();if(!Array.isArray(editor.history)||!editor.history.length)sbeHistoryInit();editor.history=editor.history.slice(0,editor.historyIndex+1);if(editor.history[editor.history.length-1]!==snap)editor.history.push(snap);if(editor.history.length>41)editor.history.shift();editor.historyIndex=editor.history.length-1;editor.undo=editor.history.slice(0,editor.historyIndex);editor.redo=[]}
function sbeRestore(snap){const v=JSON.parse(snap);editor.sablon=v.sablon;editor.planAdi=v.planAdi;editor.elements=sbeNormalize(v.elements);editor.layout=v.layout||editor.layout;editor.selection=[];sbeTableConfig();sbeTableNumber();sbeTableRender();const root=document.getElementById('transportBusEditor');const search=root?.querySelector('[data-sbe-student-search]');const list=root?.querySelector('[data-sbe-students]');if(list)list.innerHTML=sbeStudents(search?.value||'');if(root)sbeBindStudents(root);}
function sbeUndo(){if(!editor?.editable||!Array.isArray(editor.history)||editor.historyIndex<=0)return;editor.historyIndex--;sbeRestore(editor.history[editor.historyIndex]);editor.undo=editor.history.slice(0,editor.historyIndex);editor.redo=editor.history.slice(editor.historyIndex+1)}
function sbeRedo(){if(!editor?.editable||!Array.isArray(editor.history)||editor.historyIndex>=editor.history.length-1)return;editor.historyIndex++;sbeRestore(editor.history[editor.historyIndex]);editor.undo=editor.history.slice(0,editor.historyIndex);editor.redo=editor.history.slice(editor.historyIndex+1)}
...
function sbeStudents(term=''){const q=norm(term);return students(editor.servisId).filter(v=>!q||norm([v.ogrenciAdi,v.ogrenciNo,className(v.sinifId)].join(' ')).includes(q)).map(v=>{const seat=editor.elements.find(e=>String(e.studentId)===String(v.id));return {v,seat}}).sort((a,b)=>{const aa=!!a.seat,bb=!!b.seat;if(aa!==bb)return aa?1:-1;return String(a.v.ogrenciAdi||'').localeCompare(String(b.v.ogrenciAdi||''),'tr',{numeric:true})}).map(({v,seat})=>'<button type="button" draggable="true" class="sbe-student '+(seat?'is-seated ':'is-unseated ')+(editor.pendingStudentId===v.id?'is-pending ':'')+'" data-sbe-student="'+esc(v.id)+'"><span class="sbe-avatar">'+esc(String(v.ogrenciAdi||'Ö').trim().slice(0,1).toLocaleUpperCase('tr'))+'</span><span><strong>'+esc(v.ogrenciAdi||'Öğrenci')+'</strong><small>'+esc(className(v.sinifId))+(seat?' · Koltuk '+esc(seat.seatNumber):' · Yerleşmedi')+'</small></span><span class="sbe-student-status" aria-label="'+(seat?'Yerleşti':'Yerleşmedi')+'">'+(seat?'✓':'•')+'</span></button>').join('')||'<div class="ka-empty">Öğrenci bulunamadı.</div>'}
...
function sbeBindStudents(ov){ov.querySelectorAll('[data-sbe-student]').forEach(b=>{
  ...
  b.addEventListener('click',e=>{
    ...
    ov.querySelectorAll('[data-sbe-student]').forEach(x=>x.classList.toggle('is-active',x===b&&editor.pendingStudentId===sid));
    toast?.(editor.pendingStudentId?'Öğrenci seçildi. Tablodaki koltuğa veya boş hücreye dokunun.':'Öğrenci seçimi kaldırıldı.');
  });
})}
async function sbeSave(s){if(!editor?.editable)return;if(!canEditBusSeats())return;const b=document.querySelector('[data-sbe-save]');sbeNumber();sbeTableConfig();const servisId=editor.servisId,sablon=editor.sablon,elements=structuredClone(editor.elements),planAdi=editor.planAdi,layout=structuredClone(editor.layout);if(b){b.disabled=true;b.textContent='Kaydediliyor…'}try{const task=window.ServisOturmaService.planTableKaydet(servisId,sablon,elements,layout,planAdi);await Promise.resolve(task);window.SyncEngine?.schedule?.(0);if(b)b.textContent='Kaydedildi ✓';toast?.('Oturma planı kaydedildi.');closeEditor();try{render()}catch(e){console.error('[Transport/seating-render]',e)}}catch(e){if(b){b.disabled=false;b.textContent='💾 Kaydet'}console.error('[Transport/seating-save]',e);toast?.('Plan kaydedilemedi: '+(e?.message||e))}}
function busPrintReport(s){
  ...
  const totalSeats=editor.elements.filter(sbeIsSeat),seated=totalSeats.filter(e=>!!seatStudentName(e));
  const reserved=totalSeats.filter(e=>!seatStudentName(e)&&e.properties?.reserved),blankSeats=totalSeats.filter(e=>!seatStudentName(e)&&!e.properties?.reserved);
  const occupied=new Set(),cells=[];
  for(let r=1;r<=rows;r++)for(let c=1;c<=cols;c++){
    if(occupied.has(r+','+c))continue;
    const e=sbeTableCell(r,c);
    if(!e){cells.push('<div class="sbe-print-cell sbe-print-empty" style="grid-row:'+r+';grid-column:'+c+'"></div>');continue}
  ...
  const rowTemplate=rowHeights.map(v=>Math.max(52,Number(v)||52)+'px').join(' ');
  const body='<div class="sbe-print"><h1>'+esc(school.okulAdi||'KORUK İLK-ORTAOKULU')+'</h1><h2>SERVİS OTURMA PLANI</h2><p><b>Servis:</b> '+esc(serviceName(s))+' &nbsp; <b>Plaka:</b> '+esc(s.plaka||'—')+' &nbsp; <b>Şoför:</b> '+esc(s.soforAdi||'—')+'</p><p><b>Toplam koltuk:</b> '+totalSeats.length+' &nbsp; <b>Öğrenci:</b> '+seated.length+' &nbsp; <b>Boş:</b> '+blankSeats.length+' &nbsp; <b>Rezerve:</b> '+reserved.length+'</p><div class="sbe-print-table" style="grid-template-columns:'+colTemplate+';grid-template-rows:'+rowTemplate+'">'+cells.join('')+'</div></div>';
  const extra='<style>.sbe-print{font-family:Arial,sans-serif;color:var(--ka-report-transport-text);font-size:9pt}.sbe-print h1,.sbe-print h2{text-align:center;margin:2mm 0}.sbe-print p{margin:2mm 0;font-size:8pt}.sbe-print-table{display:grid;width:min(170mm,100%);margin:5mm auto 0;border:1.5px solid var(--ka-report-transport-border);border-radius:7mm;overflow:hidden;background:var(--ka-report-transport-bg);box-sizing:border-box;break-inside:avoid}.sbe-print-cell{box-sizing:border-box;min-width:0;min-height:0;border-right:1px solid var(--ka-report-transport-border-soft);border-bottom:1px solid var(--ka-report-transport-border-soft);background:var(--ka-report-transport-cell);padding:1.5mm 1mm;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;overflow:hidden}.sbe-print-empty{background:var(--ka-report-transport-bg)}.sbe-print-cell.seat{background:var(--ka-report-transport-seat)}.sbe-print-cell.filled{background:var(--ka-report-transport-filled)}.sbe-print-cell.driver{background:var(--ka-report-transport-cell)3d6}.sbe-print-cell.door{background:var(--ka-report-transport-door)}.sbe-print-cell.special{background:var(--ka-report-transport-bg)}.sbe-print-no{align-self:flex-start;font-size:6.5pt;font-weight:400}.sbe-print-icon{font-size:13pt;line-height:1}.sbe-print-cell strong{display:block;width:100%;font-size:7.2pt;line-height:1.08;max-width:100%;overflow-wrap:anywhere;word-break:break-word;white-space:normal}.sbe-print-cell small{font-size:5.8pt;margin-top:1mm}</style>';
...
function sbeApplyTableZoom(){
  const root=document.getElementById('transportBusEditor'),table=root?.querySelector('[data-sbe-table]'),wrap=root?.querySelector('[data-sbe-table-zoom]'),value=root?.querySelector('[data-sbe-table-zoom-value]');
  if(!table||!wrap)return;
  const z=Math.max(.6,Math.min(2.2,Number(editor.tableZoom)||1));editor.tableZoom=z;
  table.style.transform='scale('+z+')';table.style.transformOrigin='top left';
  requestAnimationFrame(()=>{wrap.style.width=Math.ceil(Math.max(1,table.scrollWidth)*z)+'px';wrap.style.height=Math.ceil(Math.max(1,table.scrollHeight)*z)+'px'});
  if(value)value.textContent=Math.round(z*100)+'%';
}
function sbeTableZoom(delta){if(!editor?.editable)return;editor.tableZoom=Math.max(.6,Math.min(2.2,(Number(editor.tableZoom)||1)+delta));sbeApplyTableZoom();}
function sbeTableRender(){
  ...
  host.querySelectorAll('[data-sbe-cell]').forEach(cell=>{
    const activate=e=>{
      const now=Date.now();
      if((e.type==='pointerup'||e.type==='touchend') && now-(editor._sbeLastCellActivate||0)<250)return;
      if(e.type==='click' && now-(editor._sbeLastCellActivate||0)<700)return;
      editor._sbeLastCellActivate=now;
      e.preventDefault?.();
      e.stopPropagation?.();
...
function sbeTableSelectTool(kind){
  ...
  root?.querySelectorAll('[data-sbe-student]').forEach(b=>b.classList.remove('is-active'));
...
function sbeTableResizeStart(ev,type,index){
  ...
  const end=e=>{
    ...
    window.removeEventListener('pointerup',end,true);
    window.removeEventListener('pointercancel',end,true);
    if(moved)sbePush(before)
  };
  window.addEventListener('pointermove',move,true);
  window.addEventListener('pointerup',end,true);
  window.addEventListener('pointercancel',end,true);
}
function sbeTableCellAt(r,c){
  return editor.elements.find(e=>{
    if(e?.visible===false)return false;
    const er=Number(e.row),ec=Number(e.column),rs=Math.max(1,Number(e.rowSpan)||1),cs=Math.max(1,Number(e.colSpan)||1);
    return Number.isFinite(er)&&Number.isFinite(ec)&&r>=er&&r<er+rs&&c>=ec&&c<ec+cs;
  })||null;
}
function sbeTableAssignStudentToCell(r,c){
  const sid=editor.pendingStudentId;if(!sid)return false;
  let seat=sbeTableCellAt(r,c);
  if(seat&&!sbeIsSeat(seat)){
    toast?.('Öğrenci yalnızca koltuğa yerleştirilebilir.');return true;
  }
  if(!seat){
    sbePush();
    seat=sbeTableSeat(r,c,'seat');
  }
  sbeAssign(sid,seat.id);return true;
}
...
function sbeTableBind(root,s){
  ...
  once('[data-sbe-table-del-row]',()=>sbeTableDeleteRow());
  once('[data-sbe-table-add-col]',()=>sbeTableAddCol());
  ...
  table?.addEventListener('pointerdown',e=>{
    ...
    for(let c=1;c<t.cols;c++){
      acc+=Number(t.colWidths[c-1])||0;
      if(Math.abs(x-acc)<=16){
        sbeTableResizeStart(e,'col',c);
        return;
      }
    }
    acc=0;
    for(let r=1;r<t.rows;r++){
      acc+=Number(t.rowHeights[r-1])||0;
      if(Math.abs(y-acc)<=14){
        sbeTableResizeStart(e,'row',r);
        return;
      }
    }
...
function sbeRenderTableEditor(s){
  ...
  ov.innerHTML='<section class="ka-modal sbe-modal sbe-table-editor">'+
    ...
    '<div class="sbe-table-scroll"><div class="sbe-table-zoom" data-sbe-table-zoom><div class="sbe-table" data-sbe-table></div></div></div>'+
...
function sbeBind(ov,s){const canvas=ov.querySelector('[data-sbe-canvas]');ov.querySelector('[data-sbe-save]')?.addEventListener('click',()=>sbeSave(s));ov.querySelector('[data-sbe-print]')?.addEventListener('pointerup',e=>{e.preventDefault();e.stopPropagation();busPrintReport(s)});ov.querySelector('[data-sbe-pdf]')?.addEventListener('pointerup',e=>{e.preventDefault();e.stopPropagation();busPrintReport(s)});ov.querySelector('[data-bus-report]')?.addEventListener('pointerup',e=>{e.preventDefault();e.stopPropagation();busPrintReport(s)});ov.querySelectorAll('[data-bus-close]').forEach(b=>b.addEventListener('click',closeEditor));ov.querySelector('[data-sbe-undo]')?.addEventListener('pointerup',e=>{e.preventDefault();e.stopPropagation();sbeUndo()});ov.querySelector('[data-sbe-redo]')?.addEventListener('pointerup',e=>{e.preventDefault();e.stopPropagation();sbeRedo()});ov.querySelector('[data-sbe-delete]')?.addEventListener('pointerup',e=>{e.preventDefault();e.stopPropagation();sbeDelete()});ov.querySelector('[data-bus-clear-all]')?.addEventListener('click',()=>sbeClearAll(s));ov.querySelectorAll('[data-sbe-align]').forEach(b=>b.addEventListener('click',()=>sbeAlign(b.dataset.sbeAlign)));ov.querySelectorAll('[data-sbe-add-type]').forEach(b=>{
  let lastRun=0;
  const run=e=>{
    const now=Date.now();
    if(now-lastRun<500)return;
...
.ka-report{padding:6mm 6mm!important;font-family:'Segoe UI',Arial,sans-serif!important}.ka-report-content{display:block}.trp-inspection{color:var(--ka-report-transport-black)}.trp-inspection .trp-head{text-align:center;margin-bottom:4px}.trp-inspection .trp-school{font-size:11pt;font-weight:800;letter-spacing:.1px}.trp-inspection .trp-head-1{font-size:8pt;font-weight:700;margin-top:1px}.trp-inspection .trp-head-2{font-size:10pt;font-weight:800;margin-top:1px}.trp-inspection .trp-head-3{font-size:6.8pt;font-style:italic;color:var(--ka-report-transport-gray);margin-top:1px}.ka-report .trp-info{margin:5px 0!important;font-size:7.8pt!important;table-layout:fixed}.ka-report .trp-info td{border:1px solid var(--ka-report-transport-gray)!important;padding:2.6px 4px!important;background:var(--ka-report-transport-cell)!important;color:var(--ka-report-transport-black)!important;font-size:7.8pt!important;line-height:1.12!important}.ka-report .trp-info .trp-label{font-weight:800!important;white-space:normal!important;overflow-wrap:anywhere;word-break:normal;line-height:1.08;width:22%;font-size:7.1pt!important}.ka-report .trp-info .trp-value{width:28%;font-weight:600!important;white-space:normal!important;overflow-wrap:anywhere}.ka-report .trp-info .trp-license-label{font-size:6.8pt!important;letter-spacing:-.08px}.ka-report .trp-info .trp-driver-value{line-height:1.12}.ka-report .trp-info .trp-driver-value small{display:block;font-size:6.5pt;font-weight:700;margin-top:1px}.ka-report .trp-items{font-size:7.4pt!important;table-layout:fixed}.ka-report .trp-items th{border:1.2px solid var(--ka-report-transport-gray)!important;background:var(--ka-report-transport-cell)!important;color:var(--ka-report-transport-black)!important;padding:3px 3px!important;font-size:7.4pt!important;line-height:1.1!important}.ka-report .trp-items td{border:1px solid var(--ka-report-transport-line)!important;background:var(--ka-report-transport-cell)!important;color:var(--ka-report-transport-black)!important;padding:2.7px 3.5px!important;font-size:7.2pt!important;vertical-align:middle!important;line-height:1.12!important}.ka-report .trp-items .trp-topic{width:57%;text-align:left!important;line-height:1.12}.ka-report .trp-items .trp-answer{width:7%;text-align:center}.ka-report .trp-items .trp-explain{width:29%}.trp-ref{display:block;font-size:5.9pt;color:var(--ka-report-transport-line);font-style:italic;margin-top:1px;line-height:1.05}.trp-note{font-size:6.4pt;color:var(--ka-module-muted);margin-top:5px;line-height:1.15}.trp-signatures{display:flex;justify-content:space-around;margin-top:7px;gap:10px}.trp-signature{text-align:center;min-width:0;flex:1}.trp-signature-title{font-size:7.6pt;font-weight:700;margin-bottom:6px}.trp-signature-line{border-top:1px solid var(--ka-report-transport-gray);padding-top:3px}.trp-signature-name{font-size:8pt;font-weight:700;min-height:11px}.trp-signature-role{font-size:6.8pt;color:var(--ka-report-transport-muted);margin-top:1px}.trp-duty-select{font-family:inherit;font-size:8pt;font-weight:700;border:none;border-bottom:1px solid var(--ka-report-transport-input-border);background:transparent;text-align:center;width:100%;padding:1px 0}.trp-duty-value{font-size:8pt;font-weight:700;line-height:1.08}[contenteditable="true"]{outline:1px dashed transparent}[contenteditable="true"]:focus{outline-color:var(--ka-report-transport-focus)}@media print{[contenteditable="true"]{outline:none!important}.trp-duty-select{appearance:none;border:none!important}.trp-inspection,.trp-inspection *{break-inside:avoid-page}.trp-items tr{break-inside:avoid;page-break-inside:avoid}.trp-signatures{break-inside:avoid;page-break-inside:avoid}}
</style>`}
...
.ka-report{padding:5mm 6mm!important;font-family:'Segoe UI',Arial,sans-serif!important}.trp-monthly{color:var(--ka-module-text);height:287mm!important;min-height:287mm!important;box-sizing:border-box!important;display:block!important}.trp-monthly-head{text-align:center;margin-bottom:4px}.trp-monthly-school{font-size:14pt;font-weight:800;text-transform:uppercase;letter-spacing:.4px}.trp-monthly-title{font-size:10.5pt;font-weight:700;color:var(--ka-report-transport-accent);margin-top:2px}.ka-report .trp-monthly-info{margin-bottom:5px!important;font-size:8.5pt!important;table-layout:auto}.ka-report .trp-monthly-info td{border:1px solid var(--ka-report-transport-border-soft)!important;padding:2px 6px!important;background:var(--ka-report-transport-cell)!important;color:var(--ka-report-transport-black)!important;font-size:8.5pt!important}.ka-report .trp-monthly-info .trp-label{background:var(--ka-report-transport-head)!important;font-weight:700!important;white-space:nowrap;color:var(--ka-report-transport-head-text)!important}.ka-report .trp-student-grid{border:1px solid var(--ka-report-transport-border-soft)!important;margin-bottom:5px!important;table-layout:fixed}.ka-report .trp-student-grid>tbody>tr>td{width:50%!important;padding:0!important;border:0!important;background:var(--ka-report-transport-cell)!important;vertical-align:top!important}.ka-report .trp-student-grid>tbody>tr>td+td{border-left:1px solid var(--ka-module-muted)!important}.ka-report .trp-roster{font-size:7pt!important;line-height:1.5;table-layout:fixed}.ka-report .trp-roster th{background:var(--ka-report-transport-filled)!important;color:var(--ka-report-transport-black)!important;font-weight:700!important;padding:1px 2px!important;border:0!important;border-bottom:1px solid var(--ka-module-muted)!important;border-right:1px solid var(--ka-report-transport-border-soft)!important;font-size:7pt!important}.ka-report .trp-roster td{padding:1px 3px!important;text-align:center!important;border:0!important;border-bottom:1px solid var(--ka-report-transport-border-soft)!important;border-right:1px solid var(--ka-report-transport-border-soft)!important;background:var(--ka-report-transport-cell)!important;color:var(--ka-report-transport-black)!important;font-size:7pt!important;line-height:1.5}.ka-report .trp-roster tbody tr:nth-child(even) td{background:var(--ka-report-transport-zebra)!important}.ka-report .trp-roster .trp-roster-no{width:24px!important}.ka-report .trp-roster .trp-roster-class{width:42px!important}.ka-report .trp-roster .trp-roster-name{text-align:left!important;padding-left:4px!important}.ka-report .trp-daily{font-size:6.6pt!important;table-layout:fixed;line-height:1.4;height:155mm!important;margin-bottom:0!important}.ka-report .trp-daily tbody tr{height:var(--trp-day-height)!important;min-height:var(--trp-day-height)!important}.ka-report .trp-daily th,.ka-report .trp-daily td{border:1px solid var(--ka-report-transport-border-soft)!important;padding:3.3px 2px!important;text-align:center!important;vertical-align:middle!important;font-size:6.6pt!important;color:var(--ka-report-transport-black)!important;background:var(--ka-report-transport-cell)!important}.ka-report .trp-daily .trp-date-head{background:var(--ka-report-transport-head)!important;color:var(--ka-report-transport-black)!important;font-weight:700!important;text-align:left!important;padding-left:6px!important;width:125px!important}.ka-report .trp-daily .trp-group-head{background:var(--ka-report-transport-filled)!important;color:var(--ka-report-transport-black)!important;font-weight:700!important}.ka-report .trp-daily .trp-sub-head{background:var(--ka-report-transport-head)!important;color:var(--ka-report-transport-black)!important;font-weight:600!important;font-size:6.2pt!important}.ka-report .trp-daily .trp-date{text-align:left!important;padding-left:6px!important;white-space:nowrap!important;background:var(--ka-report-transport-cell)!important}.ka-report .trp-daily .trp-weekend td,.ka-report .trp-daily .trp-holiday td{background:var(--ka-report-transport-border-soft)!important;color:var(--ka-report-transport-muted)!important;font-style:italic!important}.trp-monthly-signatures{display:flex;justify-content:space-between;padding:7px 10px 0}.trp-monthly-signature{text-align:center;min-width:130px}.trp-monthly-signature strong{font-size:8.5pt}.trp-monthly-signature div{font-size:7.5pt;color:var(--ka-report-transport-muted);margin-top:3px}@media print{.trp-monthly{-webkit-print-color-adjust:exact;print-color-adjust:exact}}
</style>`}
