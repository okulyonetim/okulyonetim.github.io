...
async function prepareLocal(){
  ...
  const defs=syncDefs(),types=[];
  Object.entries(defs).forEach(([type,col])=>{if(col){SyncEngine.register(type,col);types.push(type)}});
  if(types.length){await SyncEngine.localHydrate(types);SyncEngine.schedule(80)}
}
function shell(){return `<section class="ka-stack" data-academic-module>
  <div class="ka-row ka-row--between"><div><h2>Akademik</h2><p class="ka-muted">Sınavlar ve planlar önce cihaz verisinden gösterilir.</p></div><span id="academicCount" class="ka-badge"></span></div>
  <div class="ka-tabs" role="tablist">
    <button class="ka-tab" data-academic-tab="written" type="button">Yazılılar</button>
    <button class="ka-tab" data-academic-tab="trial" type="button">Denemeler</button>
    <button class="ka-tab" data-academic-tab="plans" type="button">Yıllık Planlar</button>
    <button class="ka-tab" data-academic-tab="calendar" type="button">Takvim</button>
  </div>
  <label class="ka-field"><span class="ka-field__label">Ara</span><input id="academicSearch" type="search" placeholder="Sınav, ders, sınıf veya plan ara…"></label>
  <div id="academicContent" class="ka-stack"></div>
</section>`}
...
window.AcademicModule={mount,unmount,render,prepareLocal};
window.addEventListener('koruk:module-ready',e=>{if(e.detail?.name==='academic')mount()});
})();
