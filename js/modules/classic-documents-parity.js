 ...
 * ve uygulama içi DokumanOkuyucu üzerine geri kurar.
...
let observer=null,scheduled=false,category='',boundRoot=null,busy=false,toolsPromise=null;
...
function activeMain(){const shell=$('[data-documents-module]'),out=$('#documentsContent');return !!(shell&&out&&!out.querySelector('[data-teblig-back]')&&!out.querySelector('[data-evrak-module]'))}
...
function ensureControls(){
  ...
  if(!bar){
    ...
    bar.className='ka-row ka-row--between ka-wrap';
    bar.dataset.classicDocumentToolbar='';
    const search=$('#documentsSearch')?.closest('.ka-field');
    search?.insertAdjacentElement('beforebegin',bar);
  }
  const opts=categories();
  bar.innerHTML=`<label class="ka-field ka-grow" style="min-width:190px"><span class="ka-field__label">Kategori</span><select data-classic-document-category><option value="">Tüm Kategoriler</option>${opts.map(k=>`<option value="${esc(k)}" ${category===k?'selected':''}>${esc(k)}</option>`).join('')}</select></label>${canEdit()?'<button class="ka-btn" type="button" data-classic-document-add>+ Doküman Ekle</button>':''}`;
  bar.querySelector('[data-classic-document-category]')?.addEventListener('change',e=>{category=e.currentTarget.value;renderList(true)});
  bar.querySelector('[data-classic-document-add]')?.addEventListener('click',openAddModal);
}
...
function ensureTools(){
  if(global.ClassicDocumentTools)return Promise.resolve(global.ClassicDocumentTools);
  if(toolsPromise)return toolsPromise;
  const load=global.AppLoader?.loadScript?.('js/modules/classic-document-tools.js');
  if(!load)return Promise.resolve(null);
  toolsPromise=Promise.resolve(load).then(()=>global.ClassicDocumentTools||null).catch(err=>{console.warn('[ClassicDocuments/tools]',err);toolsPromise=null;return null});
  return toolsPromise;
}

function closeAddModal(){
  const ov=document.querySelector('[data-classic-document-modal]');
  const form=ov?.querySelector('[data-classic-document-form]');
  if(form)global.ClassicDocumentTools?.dispose?.(form);
  ov?.remove();
}

function modalBody(){
  return `<div class="ka-row ka-wrap" data-doc-tabs>
    <button class="ka-btn ka-btn--sm" type="button" data-doc-tab="file">📎 Dosya Yükle</button>
    <button class="ka-btn ka-btn--secondary ka-btn--sm" type="button" data-doc-tab="image">🖼 Resimlerden PDF</button>
    <button class="ka-btn ka-btn--secondary ka-btn--sm" type="button" data-doc-tab="merge">🔗 PDF Birleştir</button>
    <button class="ka-btn ka-btn--secondary ka-btn--sm" type="button" data-doc-tab="url">🔗 URL Ekle</button>
  </div>
  <div data-doc-panel="file"><label class="ka-field"><span class="ka-field__label">Dosya</span><input type="file" name="dosya"><small class="ka-muted" data-doc-file-info></small></label></div>
  <div data-doc-panel="image" hidden><div class="ka-muted" data-doc-tools-image>Gelişmiş görsel araçları yükleniyor…</div></div>
  <div data-doc-panel="merge" hidden><div class="ka-muted" data-doc-tools-merge>PDF birleştirme araçları yükleniyor…</div></div>
  <div data-doc-panel="url" hidden><label class="ka-field"><span class="ka-field__label">Harici URL</span><input type="url" name="hariciUrl" placeholder="https://drive.google.com/..."><small class="ka-muted">Google Drive, Docs veya doğrudan paylaşım bağlantısı.</small></label></div>`;
}
...
function openAddModal(){
  ...
  ov.dataset.classicDocumentModal='';
  ov.innerHTML=`<form class="ka-modal" data-classic-document-form data-doc-active-tab="file"><div class="ka-modal__header"><div><h2>📁 Doküman Ekle</h2><div class="ka-muted">Eski Dokümanlar çalışma alanı; yeni local-first servis zinciriyle çalışır.</div></div><button class="ka-icon-button" type="button" data-doc-close>×</button></div><div class="ka-modal__body ka-stack"><label class="ka-field"><span class="ka-field__label">Doküman Adı *</span><input name="ad" required maxlength="180"></label><label class="ka-field"><span class="ka-field__label">Kategori</span><select name="kategori">${categories().map(k=>`<option value="${esc(k)}">${esc(k)}</option>`).join('')}</select></label><label class="ka-field"><span class="ka-field__label">Açıklama</span><input name="aciklama" maxlength="300" placeholder="Kısa açıklama..."></label>${admin()?`<label class="ka-field"><span class="ka-field__label">Görünürlük</span><select name="gorunurluk"><option value="herkes">🌐 Herkese Açık</option><option value="kisisel">🔒 Sadece Bana Özel</option></select></label>`:''}<div data-doc-source>${modalBody()}</div><div class="ka-muted" data-doc-upload-state></div></div><div class="ka-modal__footer"><button class="ka-btn ka-btn--secondary" type="button" data-doc-close>Vazgeç</button><button class="ka-btn" type="submit">💾 Kaydet</button></div></form>`;
  const form=ov.querySelector('form');
  const setTab=tab=>{
    const valid=['file','image','merge','url'].includes(tab)?tab:'file';
    form.dataset.docActiveTab=valid;
    $$('[data-doc-tab]',ov).forEach(b=>b.classList.toggle('ka-btn--secondary',b.dataset.docTab!==valid));
    $$('[data-doc-panel]',ov).forEach(p=>p.hidden=p.dataset.docPanel!==valid);
    if(valid==='image'||valid==='merge')ensureTools().then(t=>t?.mount?.(ov,{selectTab:setTab}));
  };
  $$('[data-doc-close]',ov).forEach(b=>b.onclick=closeAddModal);
  $$('[data-doc-tab]',ov).forEach(b=>b.onclick=()=>setTab(b.dataset.docTab));
  const f=form.querySelector('input[name="dosya"]'),info=ov.querySelector('[data-doc-file-info]');
  f?.addEventListener('change',()=>{const file=f.files?.[0];if(info)info.textContent=file?`${file.name} · ${bytes(file.size)}`:'';const ad=form.elements.ad;if(file&&ad&&!ad.value.trim())ad.value=file.name.replace(/\.[^.]+$/,'')});
  form.onsubmit=saveDocument;
  ov.addEventListener('click',e=>{if(e.target===ov)closeAddModal()});
  document.body.appendChild(ov);
  ensureTools().then(t=>t?.mount?.(ov,{selectTab:setTab}));
}

async function saveDocument(e){
  e.preventDefault();
  if(busy)return;
  const form=e.currentTarget,fd=new FormData(form),tab=form.dataset.docActiveTab||'file',ad=String(fd.get('ad')||'').trim(),kategori=String(fd.get('kategori')||'Diğer'),aciklama=String(fd.get('aciklama')||'').trim(),gorunurluk=admin()?String(fd.get('gorunurluk')||'herkes'):'kisisel';
  const generated=global.ClassicDocumentTools?.fileFor?.(form)||null;
  const selected=form.querySelector('input[name="dosya"]')?.files?.[0]||null;
  const file=(tab==='image'||tab==='merge')?generated:(tab==='file'?selected:null);
  const raw=tab==='url'?String(fd.get('hariciUrl')||'').trim():'';
  const hariciUrl=raw?safeUrl(raw):'';
  if(!ad)return global.toast?.('Doküman adı zorunludur.');
  if((tab==='image'||tab==='merge')&&!generated)return global.toast?.('Önce “PDF Oluştur” ile PDF üretin.');
  if(tab==='url'&&raw&&!hariciUrl)return global.toast?.('Geçerli bir URL girin.');
  if(!file&&!hariciUrl)return global.toast?.('Dosya seçin veya URL girin.');
  const btn=form.querySelector('[type="submit"]'),state=form.querySelector('[data-doc-upload-state]');
  busy=true;btn.disabled=true;
  try{
    const meta={ad,kategori,aciklama,gorunurluk};
    if(hariciUrl){
      meta.hariciUrl=hariciUrl;
      let last='';try{last=decodeURIComponent(new URL(hariciUrl).pathname.split('/').filter(Boolean).pop()||'')}catch(_){}
      meta.dosyaAdi=(!last||['view','edit','preview','open','pub','export'].includes(last.toLowerCase()))?ad:last;
    }
    await global.DokumanlarService?.dokumanEkle?.(meta,file,p=>{if(state)state.textContent=`Yükleniyor… %${p}`});
    global.toast?.(`“${ad}” kaydedildi.`);
    closeAddModal();
    renderList(true);
  }catch(err){
    const m=String(err?.message||err);
    global.toast?.(m.startsWith('depolama-siniri:')?m.slice('depolama-siniri:'.length):m==='yetkisiz'?'Bu işlem için yetkiniz yok.':'Kayıt hatası: '+m);
  }finally{busy=false;if(btn?.isConnected)btn.disabled=false}
}

async function openDocument(id){const d=docs().find(x=>x.id===id),url=safeUrl(urlOf(d));if(!d||!url)return global.toast?.('Bu dokümanın dosyası bulunamadı.');try{if(global.PermissionService?.can?.('documents.view','preview')===false)throw new Error('yetkisiz');const viewer=await global.DocumentsModule?.ensureViewer?.(),name=d.dosyaAdi||d.ad||d.baslik||'Belge';if(viewer&&(viewer.googleDocsMu?.(url)||viewer.destekliMi?.(name)||viewer.destekliMi?.(url))){await viewer.ac(url,name);return}window.open(url,'_blank','noopener')}catch(err){if(err?.message==='yetkisiz')return;window.open(url,'_blank','noopener')}}
...
global.ClassicDocumentsParity={start,stop,renderList,openAddModal,openDocument,downloadDocument};
