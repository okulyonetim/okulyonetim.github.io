...
let observer=null,scheduled=false,category='',boundRoot=null,busy=false,stabilityBound=false;
...
function bindRouteStability(){
 ...
 document.addEventListener('click',e=>{
   ...
   if(!trigger)return;const root=document.getElementById('v2ModuleRoot');if(!root)return;
   root.style.visibility='hidden';let raf=0,done=false;
   const reveal=()=>{if(done)return;done=true;cancelAnimationFrame(raf);root.style.visibility=''};
   const settle=()=>{raf=requestAnimationFrame(()=>requestAnimationFrame(reveal))};
...
function launchPdfTool(kind){
 closeAddModal();const tools=global.DocumentsPdfTools;
...
function ensureControls(){
 const shell=$('[data-documents-module]');if(!shell||!activeMain())return;
...
function actionButton(marker,id,icon,label,extra=''){
 return `<button class="ka-btn ka-btn--ghost ka-btn--sm" style="width:34px;min-width:34px;height:34px;min-height:34px;padding:0;border-radius:10px" type="button" ${marker}="${esc(id)}" title="${esc(label)}" aria-label="${esc(label)}" ${extra}>${icon}</button>`;
}
function row(d){
...
function renderList(force=false){
 if(!activeMain())return false;ensureControls();
 ...
 out.innerHTML=`<section class="ka-stack" data-classic-document-list style="gap:14px">${list.length?[...groups.entries()].sort(([a],[b])=>a.localeCompare(b,'tr')).map(([k,rows])=>`<section><div class="ka-row ka-row--between" style="padding:2px 2px 5px"><h3 style="font-size:15px">📂 ${esc(k)}</h3><span class="ka-badge">${rows.length}</span></div><div>${rows.map(row).join('')}</div></section>`).join(''):'<div class="ka-empty">Henüz doküman bulunmuyor. “+ Doküman Ekle” ile yeni kayıt ekleyebilirsiniz.</div>'}</section>`;
 const c=$('#documentsCount');if(c)c.textContent=`${list.length} kayıt`;global.PermissionService?.apply?.(out);return true;
}

function closeAddModal(){document.querySelector('[data-classic-document-modal]')?.remove()}
function openAddModal(){
 if(!canEdit()||busy)return;closeAddModal();
 ...
 const form=$('form',ov),file=form.elements.dosya,info=$('[data-doc-file-info]',ov);$$('[data-doc-close]',ov).forEach(b=>b.onclick=closeAddModal);
...
async function saveDocument(e){
 ...
 if(!ad)return global.toast?.('Doküman adı zorunludur.');if(raw&&!hariciUrl)return global.toast?.('Geçerli bir URL girin.');if(!file&&!hariciUrl)return global.toast?.('Dosya seçin veya URL girin.');
...
bindRouteStability();
global.ClassicDocumentsParity={start,stop,renderList,openAddModal,openDocument,downloadDocument,launchPdfTool,lockMobileViewport};
