/* Koruk Asistan — Mesajlaşma Modern v4
   Hafif DOM katmanı, WhatsApp benzeri mobil arayüz ve basılı tutarak mesaj silme. */
(function(){
'use strict';
if(window.__KM_MESSAGING_MODERN_V4__) return;
window.__KM_MESSAGING_MODERN_V4__=true;
...
function tekillestir(list){
  ...
  $$('.konusma-karti',list).forEach(card=>{
    ...
    const diger=(k.katilimciUidler||[]).filter(x=>x!==ben).sort().join('|');const key='p:'+diger;
...
function sohbetDurumu(){
  ...
  if(aktif&&ov.classList.contains('active')){sohbetBasliginiDuzenle();setTimeout(mesajSatirlariniHazirla,20);}
}
...
/* Mesaj DOM satırlarını mevcut mesaj dizisiyle eşleyip basılı tutma davranışı ekler. */
...
function uzunBasSilKur(){
  if(window.__KM_LONG_PRESS_BOUND__)return;window.__KM_LONG_PRESS_BOUND__=true;
  let timer=null,startX=0,startY=0,target=null;
  const temizle=()=>{if(timer){clearTimeout(timer);timer=null;}target=null;};
  document.addEventListener('pointerdown',e=>{
    const row=e.target.closest('#detayOverlay.km-chat-mode .km-msg-row');if(!row||e.target.closest('button,a,input'))return;
    target=row;startX=e.clientX;startY=e.clientY;
    timer=setTimeout(()=>{const id=target?.dataset.messageId;if(!id)return;try{navigator.vibrate?.(35);}catch(_){}target.classList.add('km-msg-selected');setTimeout(()=>target?.classList.remove('km-msg-selected'),450);if(typeof mesajTekSil==='function')mesajTekSil(id);temizle();},650);
  },true);
  document.addEventListener('pointermove',e=>{if(!target)return;if(Math.abs(e.clientX-startX)>10||Math.abs(e.clientY-startY)>10)temizle();},true);
  document.addEventListener('pointerup',temizle,true);document.addEventListener('pointercancel',temizle,true);
  document.addEventListener('contextmenu',e=>{const row=e.target.closest('#detayOverlay.km-chat-mode .km-msg-row');if(!row)return;e.preventDefault();const id=row.dataset.messageId;if(id&&typeof mesajTekSil==='function')mesajTekSil(id);},true);
}
...
function yeniKonusmaArayuzunuKur(){
  if(window.__KM_NEW_CHAT_OVERRIDE__)return;if(typeof window.yeniKonusmaModalAc!=='function'||typeof window.modalAc!=='function')return;window.__KM_NEW_CHAT_OVERRIDE__=true;
  window.yeniKonusmaModalAc=function(){
    ...
    modalAc('Yeni Mesaj',body,async()=>{
      ...
      const id=sel.value,op=sel.selectedOptions[0],ad=op.dataset.ad,foto=op.dataset.foto;const save=$('#modalKaydetBtn');if(save){save.disabled=true;save.textContent='Açılıyor…';}
      setTimeout(()=>{try{modalKapat();}catch(_){}},40);
      try{const konusmaId=await MesajlasmaService.konusmaBaslatOgretmenIle(id,ad,typeof konusmalar!=='undefined'?konusmalar:[],foto);let acildi=false;for(let i=0;i<12;i++){try{if(Array.isArray(konusmalar)&&konusmalar.some(k=>k.id===konusmaId)){mesajKonusmaAc(konusmaId);acildi=true;break;}}catch(_){}await new Promise(r=>setTimeout(r,100));}if(!acildi){try{toast('Sohbet oluşturuldu.');}catch(_){}}}
...
function kur(){
  ...
  root.classList.add('km-messaging');eskiBasligiGizle(root);servisKorumasiniKur();yeniKonusmaArayuzunuKur();uzunBasSilKur();
  if(!$('.km-msg-hero',root)){
    const hero=document.createElement('div');hero.className='km-msg-hero';hero.innerHTML='<div class="km-msg-title-row"><span class="km-msg-title-ico">'+svgChat()+'</span><div><h2>Mesajlar</h2><p>Okul içi mesajlaşma</p></div></div><div class="km-msg-stats"><div class="km-msg-stat" data-km-stat="all"><b>0</b><span>Sohbet</span></div><div class="km-msg-stat" data-km-stat="unread"><b>0</b><span>Okunmamış</span></div><div class="km-msg-stat" data-km-stat="groups"><b>0</b><span>Grup</span></div></div>';list.parentNode.insertBefore(hero,list);
...
const ov=$('#detayOverlay');if(ov)new MutationObserver(()=>{sohbetDurumu();mesajSatirlariniHazirla();}).observe(ov,{attributes:true,attributeFilter:['class']});
const body=$('#detayBody');if(body)new MutationObserver(()=>requestAnimationFrame(mesajSatirlariniHazirla)).observe(body,{childList:true,subtree:true});
