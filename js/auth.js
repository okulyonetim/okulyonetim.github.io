...
function authDinleyiciKur(){
  ...
  auth.onAuthStateChanged(async user=>{
    if(!user){sidebarHesapGuncelle(null);AKTIF_KULLANICI=null;girisEkraniGoster();return;}
    try{
      const ref=db.collection(COL.kullanicilar).doc(user.uid),snap=await ref.get();if(!snap.exists){console.error('Bu hesap için oy_kullanicilar belgesi bulunamadı:',user.uid);alert('Hesabınız için gerekli kayıt bulunamadı. Lütfen yöneticinizle iletişime geçin.');auth.signOut();return;}
      AKTIF_KULLANICI={id:snap.id,...snap.data()};sidebarHesapGuncelle(user);
      if(typeof renkUygula==='function'){try{const tercihSnap=await db.collection('oy_kullaniciTercihleri').doc(user.uid).get();if(tercihSnap.exists&&tercihSnap.data().renkPaketi)renkUygula(tercihSnap.data().renkPaketi,false)}catch(e){console.warn('Renk tercihi okunamadı:',e)}}
      if(!AKTIF_KULLANICI.aktif){onayBekleniyorGoster();return;}
      AKTIF_ROL=null;if(AKTIF_KULLANICI.rolId){try{const rolSnap=await db.collection(COL.roller).doc(AKTIF_KULLANICI.rolId).get();if(rolSnap.exists)AKTIF_ROL={id:rolSnap.id,...rolSnap.data()}}catch(e){console.warn('Rol okunamadı:',e)}}
    ...
    }catch(err){console.error('Kullanıcı belgesi kontrol edilemedi:',err);alert('Hesap bilgileri okunamadı. İnternet bağlantınızı kontrol edip tekrar deneyin.\n\n'+(err.message||''));auth.signOut();}
  });
}
