...
async function adminYeniKullaniciOlustur(kullaniciAdi,sifre,ekBilgiler){ekBilgiler=ekBilgiler||{};const email=kullaniciAdiEmaileDonustur(kullaniciAdi),yardimciAuth=_yardimciAuthAl(),cred=await yardimciAuth.createUserWithEmailAndPassword(email,sifre),uid=cred.user.uid,belge={uid,email,kullaniciAdi,ad:ekBilgiler.ad||kullaniciAdi,admin:!!ekBilgiler.admin,aktif:true,rolId:ekBilgiler.rolId||null,bagliOgretmenId:ekBilgiler.bagliOgretmenId||null,yetkiler:{},olusturmaTarihi:new Date().toISOString()};if(window.DeviceData&&window.COL?.kullanicilar){await DeviceData.set('kullanicilar',COL.kullanicilar,uid,belge,{merge:false});if(window.SyncEngine?.push)await SyncEngine.push(['kullanicilar']).catch(e=>console.warn('[adminYeniKullaniciOlustur sync]',e?.message||e));}else{await db.collection(COL.kullanicilar).doc(uid).set(belge);}await yardimciAuth.signOut();return{uid,email};}
async function adminSifreSifirlaYeniHesapla(eskiKullaniciBelgesi,yeniKullaniciAdi,yeniSifre){const yeni=await adminYeniKullaniciOlustur(yeniKullaniciAdi,yeniSifre,{ad:eskiKullaniciBelgesi.ad||eskiKullaniciBelgesi.adSoyad||yeniKullaniciAdi,rolId:eskiKullaniciBelgesi.rolId,bagliOgretmenId:eskiKullaniciBelgesi.bagliOgretmenId,admin:eskiKullaniciBelgesi.admin});const eskiId=eskiKullaniciBelgesi.id||eskiKullaniciBelgesi.uid;if(window.DeviceData&&window.COL?.kullanicilar){const eskiBelge=DeviceData.get('kullanicilar',eskiId)||{id:eskiId,uid:eskiId};await DeviceData.set('kullanicilar',COL.kullanicilar,eskiId,{...eskiBelge,aktif:false,sifreSifirlandiEskiHesap:true,yeniHesapUid:yeni.uid,yeniKullaniciAdi,guncellenmeTarihi:new Date().toISOString()},{merge:false});if(window.SyncEngine?.push)await SyncEngine.push(['kullanicilar']).catch(e=>console.warn('[adminSifreSifirla sync]',e?.message||e));}else{await db.collection(COL.kullanicilar).doc(eskiId).update({aktif:false,sifreSifirlandiEskiHesap:true,yeniHesapUid:yeni.uid,yeniKullaniciAdi});}return yeni;}
