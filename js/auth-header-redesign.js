/* ====================================================================
   KORUK ASİSTAN — HEADER + GİRİŞ EKRANI v1
   Yalnızca UI katmanı. Auth/Firebase fonksiyonlarını değiştirmez.
   ==================================================================== */
(function(){
'use strict';
if(window.__KORUK_AUTH_HEADER_V1__) return;
window.__KORUK_AUTH_HEADER_V1__=true;
...
function loginTemaDegistir(){
  ...
  const koyu=html.getAttribute('data-theme')==='dark';
...
function loginKur(){
  const ekran=$('#girisEkrani');
  if(!ekran||ekran.dataset.korukReady==='1') return !!ekran;
  const kart=$(':scope > .config-box',ekran)||$('.config-box',ekran);
  if(!kart) return false;
  ekran.dataset.korukReady='1';
  ekran.classList.add('koruk-login');
  kart.classList.add('koruk-login-card');

  const shell=document.createElement('div');
  shell.className='koruk-login-shell';
  const brand=document.createElement('section');
  brand.className='koruk-login-brand';
  brand.innerHTML=`
    <div class="koruk-login-mark">
      <img src="assets/icon-192.png" alt="Koruk İlk-Ortaokulu logosu">
      <div class="koruk-login-mark-text"><b>Koruk İlkokulu – Ortaokulu</b><span>Okul Yönetim Sistemi</span></div>
    </div>
    <div class="koruk-login-brand-copy">
      <div class="eyebrow">Koruk Asistan</div>
      <h1>Okul yönetimi, tek bir çalışma alanında.</h1>
      <p>Ders, nöbet, öğrenci, evrak ve okul süreçlerine güvenli hesabınızla erişin.</p>
    </div>
    <div class="koruk-login-brand-foot"><i></i><span>Yetkili kullanıcı erişimi · Koruk İlk-Ortaokulu</span></div>`;
  ekran.insertBefore(shell,kart);
  shell.appendChild(brand);
  shell.appendChild(kart);
  ...
  [[kullanici,'user'],[sifre,'lock']].forEach(([input,ikon])=>{
    ...
    const wrap=document.createElement('div');wrap.className='koruk-login-input';
    ...
    if(input===sifre){
      const btn=document.createElement('button');btn.type='button';btn.className='koruk-password-toggle';btn.innerHTML=svg('eye');btn.title='Şifreyi göster';btn.setAttribute('aria-label','Şifreyi göster');
      btn.addEventListener('click',()=>{const acik=sifre.type==='text';sifre.type=acik?'password':'text';btn.innerHTML=svg(acik?'eye':'eyeOff');btn.title=acik?'Şifreyi göster':'Şifreyi gizle';btn.setAttribute('aria-label',btn.title)});
...
function aktifSayfaAdi(){
  ...
  if(panel&&panel.id){
    ...
    if(tab==='panel') return 'Genel Bakış';
    const nav=document.querySelector(`.nav-tab[data-tab="${CSS.escape(tab)}"] .nt-label`);
    if(nav&&nav.textContent.trim()) return temizMetin(nav.textContent);
    const baslik=$('.page-title',panel)||$('h2',panel);
    if(baslik&&baslik.textContent.trim()) return temizMetin(baslik.textContent);
  }
  return 'Genel Bakış';
}
function headerBaslikGuncelle(){
  const el=$('#korukTopbarPage');if(el)el.textContent=aktifSayfaAdi();
}
function profilAc(){
  try{if(typeof AltNav!=='undefined'&&AltNav&&typeof AltNav.profilAc==='function'){AltNav.profilAc();return}}catch(_){}
  try{if(typeof profilVeyaSecimAc==='function')profilVeyaSecimAc()}catch(_){}
}
...
function uygulamaTemaDegistir(){
  const b=$('#temaDugmesiTopbar')||$('#temaDugmesi');
  if(b){b.click();setTimeout(()=>{const l=$('.koruk-login-theme');if(l)l.textContent=temaIkon()},0);return}
  loginTemaDegistir();
}
function profilMenuKapat(){const m=$('.koruk-user-menu');if(m)m.classList.remove('acik')}

function hesapBilgisiGuncelle(){
  const ad=($('#hesapAd')?.textContent||'').trim()||'Kullanıcı';
  const kullanici=($('#hesapEmail')?.textContent||'').trim();
  const menuAd=$('.koruk-user-menu-head b'),menuKul=$('.koruk-user-menu-head span'),ustAd=$('.koruk-header-user-name');
  if(menuAd)menuAd.textContent=ad;if(menuKul)menuKul.textContent=kullanici;if(ustAd)ustAd.textContent=ad;
  const src=$('#hesapAvatar')?.getAttribute('src');
  const avatar=$('#topbarAvatar');
  if(avatar&&src){avatar.innerHTML=`<img src="${src.replace(/"/g,'&quot;')}" alt=""><span class="koruk-avatar-status"></span>`}
}

function headerKur(){
  const top=$('.topbar');if(!top)return false;
  top.classList.add('koruk-topbar');
  const brand=$('.topbar-brand',top),brandTitle=$('.topbar-title',top);
  if(brandTitle)brandTitle.textContent='Koruk Asistan';
  if(!$('.koruk-topbar-page',top)){
    const page=document.createElement('div');page.className='koruk-topbar-page';page.innerHTML='<strong id="korukTopbarPage">Genel Bakış</strong><span>Koruk İlk-Ortaokulu</span>';
    if(brand)brand.insertAdjacentElement('afterend',page);else top.prepend(page);
  }
  ...
  if(avatar&&!avatar.closest('.koruk-user-menu-wrap')){
    ...
    const menu=document.createElement('div');menu.className='koruk-user-menu';menu.innerHTML=`
      <div class="koruk-user-menu-head"><b>Kullanıcı</b><span></span></div>
      <button type="button" data-kmenu="profile">${svg('profile')}<span>Profilim</span></button>
      <button type="button" data-kmenu="settings">${svg('settings')}<span>Ayarlar</span></button>
      <button type="button" data-kmenu="theme">${svg('theme')}<span>Temayı Değiştir</span></button>
      <button type="button" class="koruk-menu-danger" data-kmenu="logout">${svg('logout')}<span>Çıkış Yap</span></button>`;
    wrap.appendChild(menu);
    avatar.removeAttribute('onclick');
    avatar.addEventListener('click',e=>{e.stopPropagation();menu.classList.toggle('acik')});
    ...
    document.addEventListener('click',e=>{if(!wrap.contains(e.target))profilMenuKapat()});
    document.addEventListener('keydown',e=>{if(e.key==='Escape')profilMenuKapat()});
  }
  headerBaslikGuncelle();hesapBilgisiGuncelle();
  return true;
}

let raf=0;
function baslikPlanla(){if(raf)return;raf=requestAnimationFrame(()=>{raf=0;headerBaslikGuncelle();hesapBilgisiGuncelle()})}
function gozlemKur(){
  const app=$('#app');if(app){new MutationObserver(baslikPlanla).observe(app,{subtree:true,childList:true,attributes:true,attributeFilter:['class','src','style']})}
  const hesap=$('#sidebarHesap');if(hesap){new MutationObserver(baslikPlanla).observe(hesap,{subtree:true,childList:true,characterData:true,attributes:true,attributeFilter:['src','style']})}
  document.addEventListener('click',e=>{if(e.target.closest('.nav-tab,[data-tab]'))setTimeout(baslikPlanla,20)},true);
  window.addEventListener('hashchange',baslikPlanla);
}

function baslat(){loginKur();headerKur();gozlemKur()}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',baslat,{once:true});else baslat();
[100,350,900,1800].forEach(ms=>setTimeout(()=>{loginKur();headerKur();baslikPlanla()},ms));
})();
