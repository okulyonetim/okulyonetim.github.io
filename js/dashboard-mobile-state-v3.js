...
function normalizeQuick(v){let x=uniqValid(v,QUICK,4);for(const d of quickDefaults()){if(x.length>=4)break;if(!x.includes(d))x.push(d)}return x.slice(0,4)}
...
.db-state-editor{display:flex;flex-direction:column;gap:9px}.db-state-row{display:grid;grid-template-columns:34px minmax(0,1fr) 42px 42px 42px;gap:7px;align-items:center;border:1px solid var(--border);border-radius:14px;padding:9px}.db-state-row .ico{font-size:22px}.db-state-row button{height:38px;border:1px solid var(--border);background:var(--surface-2,var(--surface));color:var(--ink);border-radius:10px}.db-state-add{width:100%;display:flex;align-items:center;gap:9px;border:1px solid var(--border);border-radius:13px;padding:11px;margin:6px 0;background:var(--surface);color:var(--ink);text-align:left}.db-state-quick-list{display:grid;grid-template-columns:1fr 1fr;gap:8px}.db-state-quick-list label{display:flex;align-items:center;gap:8px;border:1px solid var(--border);border-radius:13px;padding:11px}.db-state-quick-list input{width:18px;height:18px}
