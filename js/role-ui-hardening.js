...
function normalizeQuick(ids){const allowed=quickAllowed(),valid=new Set(allowed.map(x=>x.id));let out=(Array.isArray(ids)?ids:[]).filter((x,i,a)=>valid.has(x)&&a.indexOf(x)===i).slice(0,4);if(out.length<4){for(const d of quickDefaults()){if(out.length>=4)break;if(!out.includes(d))out.push(d)}}return out}
function renderQuick(ids){const grid=$('.db4 .db4-quick');if(!grid)return false;const chosen=normalizeQuick(ids||readLocal()||quickDefaults());const map=new Map(quickAllowed().map(x=>[x.id,x]));const use=chosen.map(id=>map.get(id)).filter(Boolean);const sig=use.map(x=>x.id).join('|');
