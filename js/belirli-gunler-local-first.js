/* Koruk Asistan — Belirli Gün ve Haftalar local-first köprüsü
 * Mevcut CizelgelerService/Repository mimarisini korur.
 * Önce IndexedDB cache'i gösterir, Firestore snapshot'ı arka planda uzlaştırır.
 */
(function(){
'use strict';
if(window.__KH_BELIRLI_GUNLER_LOCAL_FIRST__) return;
window.__KH_BELIRLI_GUNLER_LOCAL_FIRST__=true;

const TYPE='belirliGunler';
const CACHE_TYPE='belirliGunler';
let installed=false;

function uid(){ return window.KorukLocalFirst?.uid?.() || window.AKTIF_KULLANICI?.uid || ''; }
function col(){ try{return COL.belirliGunler}catch(_){return 'oy_belirliGunler'} }
...
function sameCore(a,b){
  ...
  const keys=['ad','tarihBaslangic','tarihBitis','aciklama'];
  if(keys.some(k=>String(aa[k]||'')!==String(bb[k]||'')))return false;
  const ao=Array.isArray(aa.ogretmenIdler)?aa.ogretmenIdler:[], bo=Array.isArray(bb.ogretmenIdler)?bb.ogretmenIdler:[];
  const ak=Array.isArray(aa.kontroller)?aa.kontroller:[], bk=Array.isArray(bb.kontroller)?bb.kontroller:[];
  return JSON.stringify(ao)===JSON.stringify(bo)&&JSON.stringify(ak)===JSON.stringify(bk);
}
async function cached(){ if(!window.KorukLocalFirst)return []; return window.KorukLocalFirst.cached(uid(),CACHE_TYPE,[]); }
async function save(list){ if(window.KorukLocalFirst)await window.KorukLocalFirst.cache(uid(),CACHE_TYPE,list); }
async function tombs(){ if(!window.KorukLocalFirst)return {}; return window.KorukLocalFirst.tombstones(uid(),TYPE); }
function publish(list){
  try{ if(typeof belirliGunlerListesi!=='undefined') belirliGunlerListesi=list; }catch(_){}
  try{ if(typeof renderBelirliGunler==='function') renderBelirliGunler(); }catch(_){}
  try{ if(typeof renderYaklasanEtkinlikler==='function') renderYaklasanEtkinlikler(); }catch(_){}
  window.dispatchEvent(new CustomEvent('koruk:belirli-gunler-local',{detail:{count:list.length}}));
}
...
async function reconcile(remote){
  ...
  for(const l of local){
    ...
    if(l._localPending){
      ...
      if(r&&sameCore(l,r)) map.set(l.id,r); else map.set(l.id,l);
    }
  }
  const merged=[...map.values()];
  await save(merged); return merged;
}
function localId(){ return 'bg_'+Date.now().toString(36)+'_'+Math.random().toString(36).slice(2,8); }
...
function install(){
  ...
  CizelgelerRepository.kayitlariDinle=function(tip,cb,err){
    ...
    (async()=>{
      const l=await cached(), ts=await tombs();
      const safe=l.filter(x=>!ts[x.id]);
      if(alive&&safe.length) cb(safe);
    })();
    const unsub=listenOriginal(tip,async arr=>{
      const my=++seq;
      try{const merged=await reconcile(arr);if(alive&&my===seq)cb(merged)}catch(e){if(err)err(e)}
    },err);
  ...
  CizelgelerService.kayitKaydet=async function(tip,mevcutId,veri){
    ...
    const id=mevcutId||localId();
    const data={...veri};
    if(!mevcutId&&!data.eklenmeTarihi)data.eklenmeTarihi=new Date().toISOString();
    await optimistic(id,data,'upsert');
    await window.KorukLocalFirst.queue(uid(),{
      qid:'bg-save:'+id,kind:'set-doc',collection:col(),id,data:clean(data),merge:true
    });
  ...
  CizelgelerService.kayitSil=async function(tip,id){
    ...
    if(!this._yetkiKontrol(tip))throw new Error('yetkisiz');
    await optimistic(id,null,'delete');
    await window.KorukLocalFirst.tombstone(uid(),TYPE,id,true);
    await window.KorukLocalFirst.queue(uid(),{
      qid:'bg-delete:'+id,kind:'delete-doc',collection:col(),id,tombType:TYPE,tombId:id
    });
    window.KorukLocalFirst.flush();
    return true;
  };
  ...
  CizelgelerService.kontrolToggle=async function(tip,id,kontroller){
    ...
    await window.KorukLocalFirst.queue(uid(),{
      qid:'bg-toggle:'+id,kind:'update-doc',collection:col(),id,data:{kontroller:[...kontroller]}
    });
    window.KorukLocalFirst.flush();
    return true;
  };
