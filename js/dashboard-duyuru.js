...
function yerlestir(){
  const root=$('#tab-panel.kh-home .kh-shell');if(!root)return false;const d=aktifDuyuru();if(!d)return false;const dyn=$('.kh-dynamic',root);if(!dyn)return false;
  let generic=null;Array.from(dyn.children).some(el=>{if((el.textContent||'').toLocaleUpperCase('tr').includes('DUYURU')){generic=el;return true}return false});
  let kart=$('.kh-announcement',dyn);const imza=`${d.id}|${Object.keys(d.okuyanlar||{}).length}|${okudumMu(d)?1:0}|${d.baslik||''}|${d.icerik||''}`;
  if(kart&&kart.dataset.signature===imza){if(generic&&generic!==kart)generic.remove();return true}
  const wrap=document.createElement('div');wrap.innerHTML=kartHtml(d);const yeni=wrap.firstElementChild;yeni.dataset.signature=imza;if(kart)kart.replaceWith(yeni);else if(generic)generic.replaceWith(yeni);else dyn.prepend(yeni);if(generic&&generic.isConnected&&generic!==yeni)generic.remove();
  const read=$('[data-action="read"]',yeni);if(read&&!read.disabled)read.addEventListener('change',async e=>{if(!e.target.checked)return;e.target.disabled=true;try{if(typeof DuyurularService==='undefined')throw new Error('Duyuru servisi hazır değil');await DuyurularService.okunduIsaretle(d.id);yeni.classList.remove('is-unread');yeni.classList.add('is-read');const lab=$('.kh-read-check',yeni);if(lab){lab.classList.add('checked');lab.querySelector('span:last-child').textContent='Okundu olarak işaretlendi'}const st=$('.kh-announcement-status',yeni);if(st){st.className='kh-announcement-status read';st.textContent='✓ OKUNDU'}setTimeout(yerlestir,250)}catch(err){e.target.checked=false;e.target.disabled=false;try{toast('Okundu bilgisi kaydedilemedi: '+err.message)}catch(_){}}});
  $('[data-action="detail"]',yeni)?.addEventListener('click',()=>detayAc(d.id));$('[data-action="readers"]',yeni)?.addEventListener('click',()=>detayAc(d.id));return true;
}
...
let n=0,t=setInterval(()=>{if(baslat()||++n>100)clearInterval(t)},100);document.addEventListener('DOMContentLoaded',()=>setTimeout(baslat,0));window.addEventListener('focus',()=>setTimeout(yerlestir,80));
})();
