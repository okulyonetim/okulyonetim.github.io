/* ====================================================================
   Koruk Asistan — Okul Veri Tutarlılığı v5
   - Sınıf öğretmenliği için ana kaynak: siniflar[].sinifOgretmeniId
   - Öğrenci sayaçları için ana kaynak: veliler/öğrenci kayıtları
   - Yetkili kullanıcıda canlı Firestore kayıtlarını okuyup farkları kalıcı
     olarak düzeltir; öğretmen.sorumluSinif yalnız türetilmiş ayna alandır.
   ==================================================================== */
(function(){
'use strict';
...
function norm(v){
  return String(v||'').toLocaleLowerCase('tr').normalize('NFD').replace(/[\u0300-\u036f]/g,'')
    .replace(/ı/g,'i').replace(/ş/g,'s').replace(/ğ/g,'g').replace(/ü/g,'u').replace(/ö/g,'o').replace(/ç/g,'c').trim();
}
function cinsiyet(v){
  const n=norm(v);
...
function turetilenVerileriUygula(){
  arr('siniflar').forEach(s=>{
    const o=sinifOzet(s);
    if(o.gercekKayit){s.ogrenciSayisi=o.toplam;s.kizSayisi=o.kiz;s.erkekSayisi=o.erkek;}
  });
  arr('ogretmenler').forEach(o=>{o.sorumluSinif=ogretmeninSinifMetni(o.id)||'';});
  const imza=veriImzasi();
  if(imza!==sonImza){sonImza=imza;try{window.dispatchEvent(new CustomEvent('koruk:data-consistency'))}catch(_){}}
}
...
/* ---------------- CANLI FIRESTORE DENETİMİ / KALICI EŞİTLEME ---------------- */
let fsCalisiyor=false,fsIlkDenetimTamam=false,fsTimer=null;
...
async function firestoreEsitle(neden){
  if(fsCalisiyor||!firestoreHazirMi()) return false;
  fsCalisiyor=true;
  try{
    const dbx=gv('db'),col=gv('COL');
    const [ss,vs,os]=await Promise.all([
      dbx.collection(col.siniflar).get(),
      dbx.collection(col.veliler).get(),
      dbx.collection(col.ogretmenler).get()
    ]);
    const siniflar=ss.docs.map(d=>({id:d.id,...d.data()}));
    const ogrenciler=vs.docs.map(d=>({id:d.id,...d.data()}));
    const ogretmenler=os.docs.map(d=>({id:d.id,...d.data()}));

    const sayac=new Map(siniflar.map(s=>[s.id,{toplam:0,kiz:0,erkek:0}]));
    ogrenciler.forEach(v=>{
      const sid=fsOgrenciSinifId(v,siniflar);if(!sid||!sayac.has(sid))return;
      const x=sayac.get(sid);x.toplam++;
      const c=cinsiyet(v.cinsiyet??v.cinsiyeti??v.cins??v.gender);if(c==='k')x.kiz++;else if(c==='e')x.erkek++;
    });

    const ogretmenSiniflari=new Map();
    siniflar.forEach(s=>{
      if(!s.sinifOgretmeniId)return;
      if(!ogretmenSiniflari.has(s.sinifOgretmeniId))ogretmenSiniflari.set(s.sinifOgretmeniId,[]);
      ogretmenSiniflari.get(s.sinifOgretmeniId).push(s.ad||'');
    });
    ogretmenSiniflari.forEach(l=>l.sort((a,b)=>a.localeCompare(b,'tr')));

    const yazilar=[];
    let sinifFark=0,ogretmenFark=0;
    siniflar.forEach(s=>{
      const x=sayac.get(s.id)||{toplam:0,kiz:0,erkek:0};
      if(Number(s.ogrenciSayisi||0)!==x.toplam||Number(s.kizSayisi||0)!==x.kiz||Number(s.erkekSayisi||0)!==x.erkek){
        sinifFark++;
        yazilar.push({ref:dbx.collection(col.siniflar).doc(s.id),data:{ogrenciSayisi:x.toplam,kizSayisi:x.kiz,erkekSayisi:x.erkek}});
      }
    });
    ogretmenler.forEach(o=>{
      const metin=(ogretmenSiniflari.get(o.id)||[]).filter(Boolean).join(', ');
      if(String(o.sorumluSinif||'')!==metin){
        ogretmenFark++;
        yazilar.push({ref:dbx.collection(col.ogretmenler).doc(o.id),data:{sorumluSinif:metin}});
      }
    });

    for(let i=0;i<yazilar.length;i+=400){
      const batch=dbx.batch();
      yazilar.slice(i,i+400).forEach(w=>batch.set(w.ref,w.data,{merge:true}));
      await batch.commit();
    }
    fsIlkDenetimTamam=true;
    ...
    console.info('[Koruk] Firestore veri denetimi tamamlandı:',window.__korukSonFirestoreDenetimi);
    return true;
  }catch(err){
    window.__korukSonFirestoreDenetimi={tarih:new Date().toISOString(),neden:neden||'manuel',hata:err?.message||String(err)};
    console.warn('[Koruk] Firestore veri denetimi yapılamadı:',err);
    return false;
  }finally{fsCalisiyor=false;}
}
function firestoreEsitleZamanla(neden,bekleme){
  clearTimeout(fsTimer);fsTimer=setTimeout(()=>firestoreEsitle(neden),bekleme==null?700:bekleme);
}
...
function ogretmenBasliginiDuzelt(){
  const tbody=document.getElementById('ogretmenlerTablo');const ths=tbody?.closest('table')?.querySelectorAll('thead th');
...
function servisMetoduSar(ad){
  ...
  const eski=svc[ad];svc[ad]=function(){const r=eski.apply(this,arguments);if(r&&typeof r.then==='function')r.then(()=>firestoreEsitleZamanla(ad,350)).catch(()=>{});return r};svc[ad].__korukFsSync=true;return true;
}
...
const timer=setInterval(()=>{
  ...
  if(!fsIlkDenetimTamam&&firestoreHazirMi())firestoreEsitleZamanla('uygulama-acilisi',900);
  if(++deneme>120)clearInterval(timer);
},250);
[0,500,1500,3500].forEach(ms=>setTimeout(()=>{kur();turetilenVerileriUygula();sinifTablosunuTazele();ogretmenTablosunuTazele();dashboardTazele()},ms));
try{if(typeof firebase!=='undefined'&&firebase.auth)firebase.auth().onAuthStateChanged(u=>{if(u)setTimeout(()=>firestoreEsitleZamanla('oturum-acilisi',600),500)})}catch(_){}
})();
