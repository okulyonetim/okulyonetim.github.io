...
const SVG={
 ...
 settings:'<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="3" fill="none" stroke="currentColor" stroke-width="2"/><path d="M19.4 15a1.7 1.7 0 0 0 .34 1.88l.04.04-2.86 2.86-.04-.04A1.7 1.7 0 0 0 15 19.4a1.7 1.7 0 0 0-1 1.55V21h-4v-.05A1.7 1.7 0 0 0 9 19.4a1.7 1.7 0 0 0-1.88.34l-.04.04-2.86-2.86.04-.04A1.7 1.7 0 0 0 4.6 15a1.7 1.7 0 0 0-1.55-1H3v-4h.05A1.7 1.7 0 0 0 4.6 9a1.7 1.7 0 0 0-.34-1.88l-.04-.04 2.86-2.86.04.04A1.7 1.7 0 0 0 9 4.6a1.7 1.7 0 0 0 1-1.55V3h4v.05A1.7 1.7 0 0 0 15 4.6a1.7 1.7 0 0 0 1.88-.34l.04-.04 2.86 2.86-.04.04A1.7 1.7 0 0 0 19.4 9a1.7 1.7 0 0 0 1.55 1H21v4h-.05A1.7 1.7 0 0 0 19.4 15Z" fill="none" stroke="currentColor" stroke-width="1.6"/></svg>',
...
function profileInfo(){const u=user(),t=linkedTeacher(),name=t?[t.ad,t.soyad].filter(Boolean).join(' '):profileName(u),role=t?.brans||profileRole(u),photo=t?.profilFotoUrl||t?.fotoUrl||u.profilFotoUrl||u.fotoUrl||'',username=u.kullaniciAdi?`@${u.kullaniciAdi}`:(u.email||'');return{u,t,name,role,photo,username}}
function renderProfile(){closeHeaderPopover();closeMenu();setBottomActive('profile');setTitle('Profilim');const {u,t,name,role,photo}=profileInfo(),root=$('#v2ModuleRoot');if(!root)return;const initials=name.split(/\s+/).slice(0,2).map(x=>x[0]||'').join('').toLocaleUpperCase('tr');root.innerHTML=`<section class="ka-profile-page"><div class="ka-profile-hero"><div class="ka-profile-avatar">${photo?`<img src="${esc(photo)}" alt="${esc(name)}">`:esc(initials||'K')}</div><div class="ka-profile-copy"><h2>${esc(name)}</h2><p>${esc(role)}</p>${t?.telefon||u.telefon?`<small>☎ ${esc(t?.telefon||u.telefon)}</small>`:''}${t?.email||u.email?`<small>✉ ${esc(t?.email||u.email)}</small>`:''}</div></div><div class="ka-profile-section"><h3>Çizelgelerim</h3><div class="ka-profile-grid"><button type="button" data-profile-route="academic">📅<span>Ders Programım</span></button><button type="button" data-profile-route="management">🛡️<span>Nöbetlerim</span></button><button type="button" data-profile-route="academic">📏<span>Sınavlarım</span></button><button type="button" data-profile-route="documents">📄<span>Diğer Görevlerim</span></button></div></div><button type="button" class="ka-logout-button" data-profile-logout>↪ Oturumu Kapat</button></section>`;$$('[data-profile-route]',root).forEach(b=>b.addEventListener('click',()=>routeModule(b.dataset.profileRoute,{bottom:'profile'})));root.querySelector('[data-profile-logout]')?.addEventListener('click',()=>{if(confirm('Hesabınızdan çıkış yapmak istediğinize emin misiniz?'))global.cikisYap?.()})}
function currentTheme(){return document.documentElement.getAttribute('data-theme')==='dark'?'dark':'light'}
function syncThemeButton(){const dark=currentTheme()==='dark',btn=$('[data-ka-theme-toggle]');if(btn){btn.classList.remove('ka-hidden');btn.innerHTML=dark?SVG.sun:SVG.moon;btn.setAttribute('aria-label',dark?'Açık temaya geç':'Koyu temaya geç');btn.title=dark?'Açık temaya geç':'Koyu temaya geç'}global.AppStore?.set?.('ui.theme',dark?'dark':'light');const meta=$('meta[data-ka-theme-color]');if(meta)meta.content=dark?'#0d1118':'#17684f'}
...
function toggleTheme(){themeTouched=true;return applyTheme(currentTheme()==='dark'?'light':'dark')}
...
function takeThemeOwnership(){if(!global.AppLoader)return;global.AppLoader.applyTheme=applyTheme;global.AppLoader.toggleTheme=toggleTheme}
function bindThemeCapture(){document.addEventListener('click',e=>{const themeBtn=e.target.closest?.('[data-ka-theme-toggle]');if(!themeBtn)return;e.preventDefault();e.stopPropagation();toggleTheme()},true)}
...
function dateOf(x){const v=x?.tarih||x?.sonTarih||x?.baslangicTarihi||x?.olusturmaTarihi||x?.guncellenmeTarihi;if(!v)return null;try{const d=v?.toDate?v.toDate():new Date(String(v).length===10?`${v}T00:00:00`:v);return isNaN(d)?null:d}catch(_){return null}}
function notificationRows(){const now=new Date(),limit=new Date(now.getTime()+45*86400000),rows=[],me=user().uid||'';for(const x of arr('hatirlaticilar')){if(x?.tamamlandi===true)continue;const d=dateOf(x);if(!d||d<now||d>limit)continue;rows.push({type:'Etkinlik',icon:'⏰',title:x.baslik||x.ad||x.konu||x.aciklama||'Yaklaşan etkinlik',date:d,time:x.saat||'',id:x.id,page:'calendar',count:1})}if(me)for(const k of arr('konusmalar')){const unread=Number(k?.okunmayanlar?.[me]||0);if(unread<1)continue;const diger=Object.entries(k.katilimciAdlari||{}).filter(([id])=>id!==me).map(([,ad])=>ad).filter(Boolean).join(', '),title=k.grupMu?(k.grupAdi||'Grup mesajı'):(diger||'Yeni mesaj'),d=dateOf(k.sonMesaj||k)||now;rows.push({type:'Mesaj',icon:'💬',title,date:d,time:'',id:k.id,page:'messages',count:unread,meta:`${unread} okunmamış mesaj`})}if(me)for(const d of arr('duyurular')){if(d?.arsivlendi===true||d?.aktif===false||d?.okuyanlar?.[me])continue;const dt=dateOf(d)||now;rows.push({type:'Duyuru',icon:'📣',title:d.baslik||'Yeni duyuru',date:dt,time:'',id:d.id,page:'announcements',count:1,meta:'Okunmamış duyuru'})}return rows.sort((a,b)=>{const ap=a.type==='Mesaj'?0:a.type==='Duyuru'?1:2,bp=b.type==='Mesaj'?0:b.type==='Duyuru'?1:2;return ap-bp||(a.type==='Etkinlik'?a.date-b.date:b.date-a.date)}).slice(0,20)}
function formatNoticeDate(d,time=''){const ds=new Intl.DateTimeFormat('tr-TR',{day:'2-digit',month:'2-digit',year:'numeric'}).format(d);return `${ds}${time?` · ${time}`:''}`}
...
function openNotifications(){const anchor=$('[data-ka-header-notification]');if(!anchor||anchor.hidden)return;const p=popoverBase(anchor,400),rows=notificationRows(),count=rows.reduce((n,r)=>n+Math.max(1,Number(r.count)||1),0);p.innerHTML=`<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;padding:6px 6px 10px;border-bottom:1px solid var(--ka-border)"><strong style="font-size:17px">Bildirimler</strong><span class="ka-badge">${count}</span></div><div style="display:flex;flex-direction:column">${rows.length?rows.map((r,i)=>`<button type="button" class="ka-btn ka-btn--ghost" data-notice-index="${i}" style="min-height:70px;border-radius:0;justify-content:flex-start;text-align:left;border-bottom:1px solid var(--ka-border);padding:10px 8px"><span style="font-size:21px">${r.icon}</span><span style="min-width:0"><strong style="display:block;white-space:normal">${esc(r.title)}</strong><small class="ka-muted">${esc(r.meta||`${r.type} · ${formatNoticeDate(r.date,r.time)}`)}</small></span></button>`).join(''):'<div class="ka-empty" style="margin-top:10px">Yeni bildirim bulunmuyor.</div>'}</div>`;$$('[data-notice-index]',p).forEach(b=>b.addEventListener('click',()=>{const r=rows[Number(b.dataset.noticeIndex)];if(r)routeModule('communication',{bottom:'menu',page:r.page,title:r.type==='Mesaj'?'Mesajlaşma':r.type==='Duyuru'?'Duyurular':'Takvim'})}))}
...
function renderSearch(){closeHeaderPopover();closeMenu();setBottomActive('search');setTitle('Arama');const root=$('#v2ModuleRoot');if(!root)return;root.innerHTML=`<section class="ka-search-page"><div class="ka-search-box">${SVG.search}<input type="search" data-shell-search-input placeholder="İsim, sınıf, telefon, servis, evrak…" autocomplete="off"></div><div class="ka-search-chips"><span>Öğrenci</span><span>Öğretmen</span><span>Servis</span><span>Evrak</span></div><div data-shell-search-results></div></section>`;const input=root.querySelector('[data-shell-search-input]');input?.addEventListener('input',()=>renderSearchResults(root,input.value));renderSearchResults(root,'');setTimeout(()=>input?.focus(),40)}
function closeQuickNote(){document.getElementById('kaQuickNoteModal')?.remove()}
function quickNoteCloseAndRestore(){closeQuickNote();setBottomActive(global.AppStore?.get?.('ui.route')==='dashboard'?'home':'menu')}
function openTextQuickNote(){closeQuickNote();const ov=document.createElement('div');ov.id='kaQuickNoteModal';ov.className='ka-modal-backdrop ka-sheet-backdrop';ov.innerHTML=`<section class="ka-modal ka-quick-note"><div class="ka-modal__header"><div><h3>Metin Notu</h3><p class="ka-muted">Hızlıca metin notu oluşturun</p></div><button type="button" class="ka-icon-button" data-note-close aria-label="Kapat">${SVG.close}</button></div><div class="ka-modal__body ka-stack"><label class="ka-field"><span class="ka-field__label">Başlık</span><input data-note-title placeholder="Not başlığı..."></label><label class="ka-field"><span class="ka-field__label">Notunuz</span><textarea data-note-text rows="8" placeholder="Notunuzu yazın..."></textarea></label></div><div class="ka-modal__footer"><button type="button" class="ka-btn ka-btn--secondary" data-note-cancel>İptal</button><button type="button" class="ka-btn" data-note-save>Kaydet</button></div></section>`;document.body.appendChild(ov);const close=quickNoteCloseAndRestore;ov.querySelector('[data-note-close]')?.addEventListener('click',close);ov.querySelector('[data-note-cancel]')?.addEventListener('click',close);ov.querySelector('[data-note-save]')?.addEventListener('click',async()=>{const baslik=ov.querySelector('[data-note-title]')?.value.trim()||'Hızlı Not',icerik=ov.querySelector('[data-note-text]')?.value.trim();if(!icerik)return;try{await global.NotlarService?.notKaydet?.(null,{baslik,icerik});close();global.toast?.('Not kaydedildi.')}catch(e){console.error('[QuickNote/text]',e);global.toast?.('Not kaydedilemedi.')}});setTimeout(()=>ov.querySelector('[data-note-title]')?.focus(),30)}
function openChecklistQuickNote(){closeQuickNote();const ov=document.createElement('div');ov.id='kaQuickNoteModal';ov.className='ka-modal-backdrop ka-sheet-backdrop';ov.innerHTML=`<section class="ka-modal ka-quick-note"><div class="ka-modal__header"><div><h3>Yapılacaklar</h3><p class="ka-muted">Kontrol edilebilir görev listesi</p></div><button type="button" class="ka-icon-button" data-note-close aria-label="Kapat">${SVG.close}</button></div><div class="ka-modal__body ka-stack"><label class="ka-field"><span class="ka-field__label">Başlık</span><input data-note-title placeholder="Yapılacaklar başlığı..."></label><div class="ka-stack" data-check-list></div><button type="button" class="ka-btn ka-btn--secondary ka-btn--sm" data-check-add>+ Görev ekle</button></div><div class="ka-modal__footer"><button type="button" class="ka-btn ka-btn--secondary" data-note-cancel>İptal</button><button type="button" class="ka-btn" data-note-save>Kaydet</button></div></section>`;document.body.appendChild(ov);const list=ov.querySelector('[data-check-list]'),add=()=>{const row=document.createElement('div');row.className='ka-row';row.innerHTML='<input type="checkbox" disabled><input class="ka-grow" data-check-text placeholder="Görev yazın..."><button type="button" class="ka-icon-button" data-check-remove>×</button>';list.appendChild(row);row.querySelector('[data-check-remove]').onclick=()=>row.remove();row.querySelector('[data-check-text]').focus()};ov.querySelector('[data-check-add]').onclick=add;add();const close=quickNoteCloseAndRestore;ov.querySelector('[data-note-close]')?.addEventListener('click',close);ov.querySelector('[data-note-cancel]')?.addEventListener('click',close);ov.querySelector('[data-note-save]')?.addEventListener('click',async()=>{const baslik=ov.querySelector('[data-note-title]')?.value.trim()||'Yapılacaklar',maddeler=[...ov.querySelectorAll('[data-check-text]')].map(x=>x.value.trim()).filter(Boolean).map(metin=>({metin,tamamlandi:false}));if(!maddeler.length)return;try{await global.NotlarService?.notKaydet?.(null,{baslik,icerik:'',maddeler});close();global.toast?.('Yapılacaklar kaydedildi.')}catch(e){console.error('[QuickNote/checklist]',e);global.toast?.('Liste kaydedilemedi.')}})}
function openUnsupportedQuickNote(type){global.toast?.(type+' notu için eski veri modeli doğrulanmadan kayıt oluşturulmayacak.')}
function openQuickNote(){closeHeaderPopover();closeMenu();setBottomActive('note');closeQuickNote();const ov=document.createElement('div');ov.id='kaQuickNoteModal';ov.className='ka-modal-backdrop ka-sheet-backdrop';ov.innerHTML=`<section class="ka-modal ka-quick-note ka-quick-note-launcher"><div class="ka-modal__header"><div><h3>Hızlı Not</h3><p class="ka-muted">Ne eklemek istiyorsunuz?</p></div><button type="button" class="ka-icon-button" data-note-close aria-label="Kapat">${SVG.close}</button></div><div class="ka-modal__body"><div class="ka-quick-note-grid"><button type="button" data-quick-kind="text"><span>📝</span><strong>Metin Notu</strong><small>Başlık ve metin</small></button><button type="button" data-quick-kind="check"><span>☑️</span><strong>Yapılacaklar</strong><small>Kontrol edilebilir görev listesi</small></button><button type="button" data-quick-kind="draw"><span>✏️</span><strong>Çizim</strong><small>El yazısı ve serbest çizim</small></button><button type="button" data-quick-kind="image"><span>🖼️</span><strong>Görsel Not</strong><small>Fotoğraf ve açıklama</small></button><button type="button" data-quick-kind="table"><span>▦</span><strong>Tablo Notu</strong><small>Satır ve sütunlarla kayıt</small></button></div><button type="button" class="ka-quick-note-all" data-quick-kind="all"><span>📚</span><div><strong>Notlarımı Görüntüle</strong><small>Tüm notlarınıza ulaşın</small></div><b>›</b></button></div></section>`;document.body.appendChild(ov);ov.querySelector('[data-note-close]')?.addEventListener('click',quickNoteCloseAndRestore);ov.querySelector('[data-quick-kind="text"]')?.addEventListener('click',openTextQuickNote);ov.querySelector('[data-quick-kind="check"]')?.addEventListener('click',openChecklistQuickNote);for(const [kind,label] of [['draw','Çizim'],['image','Görsel'],['table','Tablo']])ov.querySelector(`[data-quick-kind="${kind}"]`)?.addEventListener('click',()=>openUnsupportedQuickNote(label));ov.querySelector('[data-quick-kind="all"]')?.addEventListener('click',()=>{quickNoteCloseAndRestore();routeModule('communication',{bottom:'menu',page:'notes',title:'Notlarım'})})}
function home(){closeHeaderPopover();closeMenu();setBottomActive('home');return routeModule('dashboard',{bottom:'home'})}
function bindHeader(){document.addEventListener('click',e=>{const homeBtn=e.target.closest('[data-ka-home-trigger]');if(homeBtn){home();return}const profileBtn=e.target.closest('[data-ka-header-profile]');if(profileBtn){e.preventDefault();openProfilePopover();return}const noticeBtn=e.target.closest('[data-ka-header-notification]');if(noticeBtn&&!noticeBtn.hidden){e.preventDefault();openNotifications()}})}
