...
const state={
  ...
  ui:{theme:'light',route:'panel',online:navigator.onLine,syncing:false,pendingWrites:0,lastSyncAt:null,syncError:null},
...
function firestoreSafeValue(value,seen=new WeakSet()){if(value===undefined)return undefined;if(value===null||typeof value!=='object')return value;if(value instanceof Date)return value;if(seen.has(value))throw new Error('firestore-circular-data');seen.add(value);if(Array.isArray(value)){const out=value.map(v=>{const x=firestoreSafeValue(v,seen);return x===undefined?null:x});seen.delete(value);return out}const proto=Object.getPrototypeOf(value);if(proto!==Object.prototype&&proto!==null){seen.delete(value);return value}const out={};for(const [k,v] of Object.entries(value)){const x=firestoreSafeValue(v,seen);if(x!==undefined)out[k]=x}seen.delete(value);return out}async function runWrite(op){if(!window.db)throw new Error('db-yok');const ref=op.id?db.collection(op.collection).doc(op.id):null;if(op.kind==='delete-doc')return ref.delete();if(op.kind==='set-doc')return ref.set(firestoreSafeValue(op.data),{merge:!!op.merge});if(op.kind==='update-doc')return ref.set(firestoreSafeValue(op.data),{merge:true});if(op.kind==='delete-query'){const s=await db.collection(op.collection).where(op.field,'==',op.value).get();if(s.empty)return;const b=db.batch();s.docs.forEach(d=>b.delete(d.ref));return b.commit()}throw new Error('op-bilinmiyor')}
function flushWrites(){if(!navigator.onLine)return Promise.resolve();if(flushPromise)return flushPromise;const u=uid();if(!u)return Promise.resolve();flushing=true;flushPromise=(async()=>{const q=await get(key(u,'queue'),[]),failed=new Map(),snapshotIds=new Set((Array.isArray(q)?q:[]).map(op=>op.qid));for(const original of Array.isArray(q)?q:[]){const op={...original};try{await runWrite(op);if(op.tombType&&op.tombId)await tombstone(u,op.tombType,op.tombId,false)}catch(e){op.tries=(op.tries||0)+1;op.lastError=String(e?.message||e);op.lastTryAt=Date.now();failed.set(op.qid,op)}}const finalQueue=await mutateQueue(u,current=>{const next=[];for(const op of current){if(!snapshotIds.has(op.qid)){next.push(op);continue}const retry=failed.get(op.qid);if(retry)next.push(retry)}return next});const firstError=[...failed.values()][0]?.lastError||null;AppStore.set('ui.syncError',firstError);AppStore.set('ui.pendingWrites',finalQueue.length);window.dispatchEvent(new CustomEvent('koruk:sync-state',{detail:{pending:finalQueue.length}}));if(snapshotIds.size&&finalQueue.length===0&&navigator.onLine)setTimeout(()=>{try{scheduleSync(80)}catch(_){}},0);return finalQueue.length})().finally(()=>{flushing=false;flushPromise=null});return flushPromise}
...
async function applyRemoteBatch(u,fetched,started,{source='pull'}={}){const blocked=await pendingDataTypes(u),data={},accepted=[];let skippedLocal=0;for(const[name,rows]of Object.entries(fetched||{})){if(dataRevision(name)!==started[name]||deviceWriteActive(name)){skippedLocal++;continue}if(name!=='yemekMenuleri'&&blocked.has(name)){skippedLocal++;continue}if(name==='yemekMenuleri'){const localRows=deviceList(name);if(localRows.length){const localById=new Map(localRows.map(x=>[String(x?.id||''),x]));rows=(Array.isArray(rows)?rows:[]).map(remote=>{const local=localById.get(String(remote?.id||''));if(!local)return remote;const mergedMenu={...(remote?.menu||{})};Object.entries(local?.menu||{}).forEach(([day,value])=>{const remoteDay=mergedMenu[day];const localItems=Array.isArray(value?.items)?value.items.map(v=>String(v??'').trim()).filter(Boolean):[];const localLegacy=[value?.corba,value?.ana,value?.yardimci,value?.tatli].map(v=>String(v??'').trim()).filter(Boolean);const remoteItems=Array.isArray(remoteDay?.items)?remoteDay.items.map(v=>String(v??'').trim()).filter(Boolean):[];const remoteLegacy=[remoteDay?.corba,remoteDay?.ana,remoteDay?.yardimci,remoteDay?.tatli].map(v=>String(v??'').trim()).filter(Boolean);if(localItems.length||localLegacy.length)mergedMenu[day]=value;else if(!remoteDay||(!remoteItems.length&&!remoteLegacy.length))mergedMenu[day]=value});return{...remote,menu:mergedMenu}});const remoteIds=new Set(rows.map(x=>String(x?.id||'')));localRows.forEach(local=>{if(!remoteIds.has(String(local?.id||'')))rows.push(local)});if(!rows.length){skippedLocal++;continue}}}data[name]=rows;accepted.push(name)}if(accepted.length){accepted.forEach(markDataRevision);await cacheMany(u,data,{markWrite:false});AppStore.setDataMany(data);try{window.dispatchEvent(new CustomEvent('koruk:sync-partial',{detail:{source,types:[...accepted]}}))}catch(_){}}return{updated:accepted.length,skippedLocal}}
...
async function sync(types){while(syncing)await new Promise(resolve=>setTimeout(resolve,50));const names=syncNames(types),baseline=revisionSnapshot(names);await flushWrites();return pull(names,baseline)}
