/* Koruk Asistan - Local First Sync v2
...
async function queue(uid,op){const k=key(uid,'queue'),q=await get(k,[]);const qid=op.qid||`${Date.now()}-${Math.random().toString(36).slice(2)}`;const next={...op,qid,createdAt:op.createdAt||Date.now(),tries:op.tries||0};const i=q.findIndex(x=>x.qid===qid);if(i>=0)q[i]=next;else q.push(next);await set(k,q);schedule();return next}
...
async function run(op){if(!window.db)throw new Error('db-yok');const ref=op.id?db.collection(op.collection).doc(op.id):null;if(op.kind==='delete-doc')return ref.delete();if(op.kind==='set-doc')return ref.set(op.data,{merge:!!op.merge});if(op.kind==='update-doc')return ref.set(op.data,{merge:true});if(op.kind==='delete-query'){const s=await db.collection(op.collection).where(op.field,'==',op.value).get();if(s.empty)return;const b=db.batch();s.docs.forEach(d=>b.delete(d.ref));return b.commit()}throw new Error('op-bilinmiyor')}
async function flush(){if(flushing||!navigator.onLine)return;const u=uid();if(!u)return;flushing=true;const k=key(u,'queue');try{const q=await get(k,[]),left=[];for(const op of q){try{await run(op);if(op.tombType&&op.tombId)await tombstone(u,op.tombType,op.tombId,false)}catch(e){op.tries=(op.tries||0)+1;op.lastError=String(e?.message||e);op.lastTryAt=Date.now();left.push(op)}}await set(k,left);window.dispatchEvent(new CustomEvent('koruk:sync-state',{detail:{pending:left.length}}))}finally{flushing=false}}
let timer=null;function schedule(){clearTimeout(timer);timer=setTimeout(flush,350)}
window.addEventListener('online',()=>setTimeout(flush,150));document.addEventListener('visibilitychange',()=>{if(!document.hidden)flush()});setInterval(flush,12000);
