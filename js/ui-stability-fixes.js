...
/* Davranış/kararlılık scriptleri — tema üretmezler. */
...
scriptYukle('js/rapor-preview.js?v=1','rapor-preview');
...
/* TEK aktif uygulama tema kaynağı:
   index.html'de doğrudan bağlı css/modal-interaction-fix.css.
   Eski sayfa-tema CSS'leri burada artık YÜKLENMEZ. */

/* app-pages-theme.js yalnız mevcut kararlı modal/sınıf işaretleme davranışı için
   korunuyor; görsel tema kaynağı değildir. */
...
function kullaniciHazir(){
  ...
  if(u.bagliOgretmenId){
    try{
      ...
      return Array.isArray(og)&&og.some(x=>x&&x.id===u.bagliOgretmenId);
...
function anaSayfaBaslat(){
  ...
  stilYukle('css/dashboard-home-neutral.css?v=1','dashboard-home-neutral');
  stilYukle('css/dashboard-home-colors.css?v=7','dashboard-home-colors');
  scriptYukle('js/dashboard-home.js?v=6','dashboard-home');
  scriptYukle('js/dashboard-duyuru.js?v=2','dashboard-duyuru');
...
if(mobil){
  let deneme=0;
  const timer=setInterval(()=>{if(anaSayfaBaslat()||++deneme>300)clearInterval(timer)},100);
  document.addEventListener('DOMContentLoaded',()=>setTimeout(anaSayfaBaslat,0));
  window.addEventListener('load',()=>setTimeout(anaSayfaBaslat,100));
}

scriptYukle('js/role-ui-hardening.js','role-ui-hardening');

/* Döküman yükleme modalındaki Kaydet butonu davranışı — mevcut işlev korunur. */
