...
function kur(){
  ...
  window.dokumanYukleModalAc=function(){
    ...
    requestAnimationFrame(()=>{
      ...
      if(!b)return;
      // Aynı global modal düğmesi önceki işlemden disabled/display durumu taşımamalı.
      b.style.display='';
      b.disabled=false;
      b.removeAttribute('aria-disabled');
      b.textContent='💾 Kaydet';
      // PDF araçları kendi iş akışında gerektiğinde düğmeyi yeniden pasifleştirir.
      const pdfMod=window._dokPdfIslemModu||'';
      if(pdfMod==='resim'||pdfMod==='birlestir'){
        // wrapper modu hemen temizlediği için görünür panellerden gerçek modu belirle.
