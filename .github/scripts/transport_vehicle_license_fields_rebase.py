...
def patch_transport(s):
    ...
    if old_detail not in s: raise SystemExit('service detail anchor missing')
    ...
    if old_modal not in s: raise SystemExit('service modal anchor missing')
    ...
    if old_payload not in s: raise SystemExit('service payload anchor missing')
    ...
    if old_css not in s: raise SystemExit('inspection css anchor missing')
    ...
    if old_label not in s: raise SystemExit('inspection label anchor missing')
    return s.replace(old_label,new_label,1)
...
def patch_test(s):
    ...
    return s.replace("js/modules/transport.js?v=893","js/modules/transport.js?v=894")
...
def patch_sw(s):
    s=s.replace("const CACHE_ADI='oy-cache-v945';","const CACHE_ADI='oy-cache-v946';")
