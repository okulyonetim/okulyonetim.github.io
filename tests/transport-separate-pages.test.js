...
for(const token of ['TransportReports','SABIT_LISTE_BOYU=30','DENETIM_MADDELERI','TAŞIMA YOLUYLA EĞİTİME ERİŞİM YÖNETMELİĞİ KAPSAMINDA HİZMET SUNAN','(TAŞIMA MERKEZİ OKUL/KURUM MÜDÜRLÜĞÜNCE KULLANILACAK)','ARACIN MODEL YILI','SÜRÜCÜ BELGESİNİN ALINDIĞI YIL VE SINIFI','class="trp-duty-select"','contenteditable="true"','global.ServisOturmaRepository?.planServisIdIleGetir','👑','trp-student-grid',"'<td>Hafta Sonu</td>'.repeat(8)","'<td>Resmî Tatil</td>'.repeat(8)",'ReportEngine.documentHtml','ReportEngine.previewHtml','ReportEngine.printHtml','cloneNode(true)','global.TransportReports={prepare,denetim,takip,takipSec,reportStudents}',"requireReport('transport.report.inspection')","requireReport('transport.report.monthly')"])assert(transport.includes(token),`Taşıma resmî rapor fidelity sözleşmesi eksik: ${token}`);
...
assert(loader.includes("define('transport',['js/modules/report-engine.js','js/modules/transport.js?v=917'])"),'Transport yalnız canonical UI + ortak ReportEngine ile lazy yüklenmeli.');
