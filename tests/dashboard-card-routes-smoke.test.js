...
assert(dash.includes("function teacherShell(){return`${cardVisible('welcome')?hero():''}${statsSection()}${announcementSection()}${pollSection()}${trialCounterSection()}"),'Öğretmen ana sayfası okul özeti/duyuru/anket/deneme akışını ve öğretmen yemek görünürlüğünü taşımalı.');
...
assert(dash.includes('function bellTimeline(live)')&&dash.includes('kh-bell-timeline')&&dash.includes('Number(live?.progress)')&&dash.includes('kh-bell-progress-v2'),'Canlı zil kartı canonical progress verisini mobil zaman çizgisi ve gerçek ilerleme çubuğunda kullanmalı.');
...
const newsViewportStart=css.indexOf('.ka-home .kh-news-viewport{'),newsViewportEnd=css.indexOf('}',newsViewportStart),newsViewportCss=css.slice(newsViewportStart,newsViewportEnd+1);
...
assert(css.includes('animation:khTicker var(--kh-ticker-time,28s) linear infinite')&&css.includes('animation-delay:var(--kh-ticker-delay,0s)')&&!css.includes('.ka-home .kh-news.is-ready .kh-news-track{animation:khTicker')&&!css.includes('.kh-news:hover .kh-news-track{animation-play-state:paused}'),'Haber bandı DOMa gelir gelmez ve yeniden render sonrasında kesintisiz akmalı.');
...
assert(dash.includes('kh-bell-timeline-v2')&&dash.includes('kh-bell-progress-v2')&&!dash.includes('GÜN İÇİ AKIŞ')&&!dash.includes('ka-tab kh-bell-node'),'Canlı zil kartı mobil timeline ve gerçek progress ile kompakt kalmalı.');
...
assert(dash.includes("function trialCounterSection(){const list=trialLiveRows().slice(0,1);if(!list.length)return'';"),'Aktif deneme sınavı sayacı yoksa sayaç kartı hiç render edilmemeli.');
...
assert(dash.includes('data-news-signature')&&!dash.includes('freshNews.replaceWith(oldNews)')&&dash.includes('class=\"kh-news-loop\"')&&dash.includes('--kh-ticker-delay:${tickerDelay.toFixed(2)}s'),'Kayan haber yeniden render sırasında eski DOMu söküp takmamalı; saat fazlı çift şeritle kesintisiz akmalı.');
...
assert(dash.includes('kh-bell-holiday-countdown')&&dash.includes('holidayProgress')&&dash.includes('holidayBellSvg')&&dash.includes('live?.message'),'Tatil zil kartı ayrı tatil ikonu, kalan gün sayacı, ilerleme ve tatil mesajı göstermeli.');
