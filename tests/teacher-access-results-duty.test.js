...
assert(settings.includes("settingsTeacherUser()&&['users','statistics','roles','admin-info'].includes(tab)"),'Öğretmen Kullanıcı İşlemleri, İstatistik, Roller ve İdari Bilgiler ayarlarını görememeli.');
assert(settings.includes("if(tab==='admin-info')return adminInfoAllowed()")&&settings.includes('function adminInfoAllowed()'),'İdari Bilgiler sayfası ayrıca Admin/Yönetici yetki kontrolünden geçmeli.');
