...
assert(settings.includes("if(tab==='admin-info')return false")&&settings.includes("adminPanel=adminInfoAllowed()?adminInfoPanel():''")&&settings.includes('function adminInfoAllowed()'),'İdari bilgiler ayrı route olmadan yalnız Admin/Yönetici için Okul Bilgileri içinde görünmeli.');
assert(settings.includes('function socialRow(x={},editable=false)')&&settings.includes('function socialSettings()')&&settings.includes('sosyalKaydet(sosyalLinkler)')&&settings.includes("if(!window.OkulBilgileriService?._duzenleyebilir?.())"),'Sosyal bağlantılar ayrı Settings sayfasında yetki kontrollü olmalı.');
...
assert(assetVersion(index,'css/design-system.css')>=878&&assetVersion(index,'js/core/shell-ui.js')>=883&&assetVersion(index,'js/app-loader.js')>=897,'Değişen kabuk kaynakları v876 ile cache-bust edilmeli.');
assert(assetVersion(loader,'js/modules/academic.js')>=877&&assetVersion(loader,'js/modules/settings.js')>=880,'Değişen lazy modüller v876 ile yüklenmeli.');
assert(cacheVersion(sw)>=911&&assetVersion(sw,'js/modules/settings.js')>=880,'Service Worker v876 kaynaklarını önbelleğe almalı.');
