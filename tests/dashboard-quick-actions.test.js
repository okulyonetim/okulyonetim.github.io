...
assert(dashboard.includes("function teacherShell(){return`${cardVisible('welcome')?hero():''}${foodMenuSection()}${statsSection()}${announcementSection()}${pollSection()}${trialCounterSection()}${newsSection()}${teacherFocusSection()}${lessonsSection()}${dutySection()}${examsSection()}${upcomingSection()}${quickSection()}${calendarSection()}${notesSection()}${allTodayDutySection()}${weekDutySection()}${socialSection()}`}"),'Öğretmen ana sayfası güncel rol sırasını kullanmalı.');
