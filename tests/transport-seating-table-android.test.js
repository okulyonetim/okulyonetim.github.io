...
if(!js.includes("querySelector('[data-sbe-merge]')?.addEventListener('pointerup'")) throw new Error('Birleştir Android pointerup kullanmalı.');
...
if(!js.includes("const colTemplate=colWidths.map(v=>Math.max(70,Number(v)||70)+'px').join(' ')")) throw new Error('Rapor sütunları editör ölçülerini px olarak kullanmalı.');
if(!loader.includes("js/modules/transport.js?v=925")) throw new Error('Transport cache sürümü yenilenmemiş.');
if(!sw.includes("./js/modules/transport.js?v=925")) throw new Error('Service Worker transport sürümü yenilenmemiş.');
