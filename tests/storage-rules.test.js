...
async function main(){
  ...
  try {
    ...
    await testEnv.withSecurityRulesDisabled(async (context) => {
      ...
      await setDoc(doc(db, 'oy_konusmalar', 'k1'), {
        katilimciUidler:['teacherUid','teacher2Uid'], grupMu:false
      });
    });
    ...
    const mesajRef = ref(teacherStorage, 'mesajDosyalari/k1/test.pdf');

    await assertSucceeds(uploadBytes(mesajRef, pdfData, { contentType:'application/pdf' }));
    ...
    // Konuşma dışındaki kullanıcı mesaj dosyası yükleyemez.
    await assertFails(uploadBytes(
      ref(outsiderStorage, 'mesajDosyalari/k1/sahte.pdf'),
      pdfData,
    ...
    await assertFails(uploadBytes(
      ref(teacherStorage, 'mesajDosyalari/k1/sahte.exe'),
      new Uint8Array([1,2,3]),
      { contentType:'application/octet-stream' }
    ));

    // Mevcut diğer Storage yolları giriş yapmış kullanıcılar için çalışmaya devam eder.
    await assertSucceeds(uploadBytes(
      ref(teacherStorage, 'duyurular/test.png'),
      new Uint8Array([1,2,3]),
      { contentType:'image/png' }
    ));
    await assertFails(uploadBytes(
      ref(anonStorage, 'duyurular/anon.png'),
      new Uint8Array([1,2,3]),
      { contentType:'image/png' }
    ));
