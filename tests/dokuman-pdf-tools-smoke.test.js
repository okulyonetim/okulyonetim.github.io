...
assert(ui.includes("b.removeAttribute('aria-disabled')"), 'Kaydet düğmesinin aria-disabled durumu temizlenmeli.');
...
assert(contrast.includes('.btn-ghost:hover'), 'Ghost buton hover kontrastı normalize edilmeli.');
assert(contrast.includes('.btn-ghost:focus-visible'), 'Ghost buton focus kontrastı normalize edilmeli.');
assert(contrast.includes('input::placeholder'), 'Placeholder kontrastı normalize edilmeli.');
assert(contrast.includes('select option'), 'Select seçenekleri tema kontrastını korumalı.');
assert(contrast.includes('.detay-head .btn-ghost'), 'Renkli başlıktaki ghost butonlar beyaz metni korumalı.');
