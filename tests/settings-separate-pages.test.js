...
assert(settings.includes("['social','Sosyal Medya'")&&settings.includes("active==='social'?socialSettings()"),'Sosyal medya Okul Bilgilerinden ayrı Settings sayfası olmalı.');
assert(settings.includes('id=\"schoolAddress\"')&&settings.includes("adminPanel=adminInfoAllowed()?adminInfoPanel():''")&&!settings.includes("['admin-info','İdari Bilgiler ve Şifreler'"),'Okul Bilgileri adresi ve yetkili idari paneli birlikte taşımalı; ayrı admin-info menüsü kalmamalı.');
for(const page of ['school','social','lesson-hours','holiday','lesson-list','branch-list','users','statistics','account','sync','roles','app','reminders','storage'])assert(settings.includes(`'${page}'`),`Settings sayfa hedefi eksik: ${page}`);
assert(settings.includes("['lesson-hours','Ders Saatleri','Zil saatleri ve öğle arası']")&&settings.includes('data-lesson-auto-first')&&settings.includes('data-lesson-auto-duration')&&settings.includes('data-lesson-auto-break')&&settings.includes('data-lesson-auto-lunch-duration')&&settings.includes('data-lesson-auto-distribute'),'Ders Saatleri canonical ekranda yalnız zil/öğle arası otomatik dağıtım parametrelerini sağlamalı.');
...
assert(settings.includes("['holiday','Tatil Modu','Planlı tatiller ve otomatik tarih aralıkları']")&&settings.includes("active==='holiday'?holidayPage()")&&settings.includes('data-holiday-start')&&settings.includes('data-holiday-end')&&settings.includes('tatilAraliklari')&&settings.includes('Tatil tarih aralıkları birbiriyle çakışamaz.')&&!settings.includes('data-lesson-holiday'),'Tatil Modu Ders Saatlerinden ayrı native sayfada çoklu tarih aralıklarını yönetmeli.');
...
assert(shell.includes("name==='settings'&&['school','social','lesson-hours','holiday','users','statistics','account','sync','roles','app','reminders','storage'].includes(page)"),'Shell Settings alt sayfalarını, Tatil Modu ve Sosyal Medya dahil, doğrudan route etmeli.');
