...
# Classic shell lazy-loader checks should validate module order/ownership, not a particular cache-bust number.
old="const optionalLoaderSource=fs.readFileSync('js/app-loader.js','utf8');"
new=old+"\nconst normalizeLoaderBundle=v=>String(v||'').replace(/\\?v=\\d+/g,'');"
if old not in s:
    raise SystemExit('Classic shell optionalLoaderSource bulunamadı')
s=s.replace(old,new,1)
...
if old not in s:
    raise SystemExit('Classic shell dashboard exact loader assertion bulunamadı')
s=s.replace(old,new,1)
...
if old not in s:
    raise SystemExit('Classic shell academic versioned assertion bulunamadı')
s=s.replace(old,new,1)
...
if old not in s:
    raise SystemExit('Classic shell communication versioned assertion bulunamadı')
s=s.replace(old,new,1)
p.write_text(s,encoding='utf-8')
