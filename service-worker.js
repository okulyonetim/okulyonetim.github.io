...
self.addEventListener('notificationclick',event=>{event.notification.close();const target=event.notification.data?.url||'./';event.waitUntil(clients.matchAll({type:'window',includeUncontrolled:true}).then(windows=>{for(const w of windows){if('focus'in w){if('navigate'in w)w.navigate(target);return w.focus()}}return clients.openWindow?clients.openWindow(target):null}))});
